...
# usage : 'python zephyr_module.py <stm32_serie>'
...
if not module_serie or (not args['a1'] and not args['a2'] and not args['a3']):
    ...
    print " --> example : zephyr_module stm32f3"
    sys.exit()
...
if pos_version:
    ...
    make_module.cleanup(module_serie)
    sys.exit()
...
previous_module = make_module.module(module_serie)
...
latest_module = make_module.module(module_serie)
...
make_module.readme(module_serie, latest_version, latest_commit)
make_module.makelist(module_serie)
make_module.release_note(module_serie)
...
# 7) build the patch list : in the new_repo
print 'Building patch from ' + previous_version + ' to ' + latest_version
# 7.1) generate a patch for each file in the module

if os.system('git diff  HEAD^ >> new_version.patch') == 0:
    os.system('dos2unix new_version.patch')

# 7.2) applying the module.patch to the latest version
#      will give the new module ready for zephyr
git_cmd = 'git apply --recount --reject ' + os.path.join(new_repo, 'module.patch') + ' &>module.log'
if not os.system(git_cmd):
    make_module.reject(module_serie)
else:
    option_force = True
    os.system('git add * && git reset -- *.patch && git reset -- *.log')

if option_force:
    os.system('git commit -asm \"stm32cube: to_new_version"')

# 8) all modules are now created on this new repo
# 8.1) apply this patch list to the zephyr module
print "Applying patch to zephyr module"
module_path = os.path.join(os.getenv('ZEPHYR_BASE'), '../modules/hal/stm32')
os.chdir(module_path)

# 8.2) choose to copy from repo
...
# 8.3) add new files with patch applied
os.system('git add *')

# 8.4) merge
if option_force:
        print "git apply patch to " + latest_version
        # 9) commit this version
        git_cmd = 'git commit -asm \"stm32cube: update ' + module_serie + ' cube version\n'
