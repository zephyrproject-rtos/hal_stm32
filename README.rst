...
database for HAL1 based series and STM32Cube Device Family Pack (DFP)
pinout files for HAL2 based series.

*-pinctrl.dtsi files are generated using generation scripts available in this
repo under scripts/genpinctrl.

HAL1 based series can be generated by running::

   python3 scripts/genpinctrl/genpinctrl.py -p /path/to/stm32-open-pin-data-repository

HAL2 based series can be generated by running::

   python3 scripts/genpinctrl/genpinctrl_hal2.py -p /path/to/stm32cube-dfp-repositories

The HAL2 input path contains one DFP repository per supported STM32Cube
package, using the ``<stm32cube-package>_dfp`` directory name expected by the
generator. The STM32Cube package URL and commit used to resolve the matching
DFP repository and commit are documented in each ``stm32cube/*/README`` file.

New set of -pinctrl.dtsi files could be generated following availability of a
new version of STM32 Open Pin Data, STM32Cube DFP pinouts or a change in the
generation script configuration files.
