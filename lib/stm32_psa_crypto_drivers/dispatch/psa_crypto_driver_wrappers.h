...
#endif
/* Headers for stm32_hal transparent driver */
#if defined(STM32_HAL_PSA_DRIVER_ENABLED)
#include "../stm32_psa_transparent_drivers/interface/include/stm32_hal_psa_driver_interface.h"
...
#define STM32_HAL_TRANSPARENT_DRIVER_ID (5)
...
static inline psa_status_t psa_driver_wrapper_init( void )
{
    ...
    status = stm32_hal_transparent_aes_init( );
    if( status != PSA_SUCCESS )
        return status;
#endif
    ...
    size_t *output_length )
{
    ...
    status = stm32_hal_transparent_cipher_encrypt
        (attributes, key_buffer, key_buffer_size, alg, iv, iv_length,
                        input, input_length, output, output_size, output_length
    );
    ...
    size_t *output_length )
{
    ...
    status = stm32_hal_transparent_cipher_decrypt
        (attributes, key_buffer, key_buffer_size, alg,
                        input, input_length, output, output_size, output_length
    );
    ...
    psa_algorithm_t alg )
{
    ...
    switch( location )
    {
        ...
        status = mbedtls_test_transparent_cipher_encrypt_setup
            (&operation->ctx.mbedtls_test_driver_ctx,
                            attributes, key_buffer, key_buffer_size, alg
        );
        if(status == PSA_SUCCESS)
        ...
        status = stm32_hal_transparent_cipher_encrypt_setup
            (&operation->ctx.stm32_hal_driver_ctx,
                            attributes, key_buffer, key_buffer_size, alg
        );
        if(status == PSA_SUCCESS)
            operation->id = STM32_HAL_TRANSPARENT_DRIVER_ID;
    ...
    psa_algorithm_t alg )
{
    ...
    status = stm32_hal_transparent_cipher_decrypt_setup
        (&operation->ctx.stm32_hal_driver_ctx,
                        attributes, key_buffer, key_buffer_size, alg
    );
    if(status == PSA_SUCCESS)
    ...
    size_t iv_length )
{
    ...
    status = mbedtls_test_transparent_cipher_set_iv
        (&operation->ctx.mbedtls_test_driver_ctx,
                        iv, iv_length
    );
...
#if defined(STM32_HAL_PSA_AES_CIPHER_DRIVER_ENABLED) 
    case STM32_HAL_TRANSPARENT_DRIVER_ID:


    status = stm32_hal_transparent_cipher_set_iv
        (&operation->ctx.stm32_hal_driver_ctx,
                        iv, iv_length
    );
    ...
    size_t *output_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_cipher_update
            (&operation->ctx.mbedtls_test_driver_ctx,
                            input, input_length, output, output_size, output_length
        );
        ...
        status = stm32_hal_transparent_cipher_update
            (&operation->ctx.stm32_hal_driver_ctx,
                            input, input_length, output, output_size, output_length
        );
    ...
    size_t *output_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_cipher_finish
            (&operation->ctx.mbedtls_test_driver_ctx,
                            output, output_size, output_length
        );
        ...
        status = stm32_hal_transparent_cipher_finish
            (&operation->ctx.stm32_hal_driver_ctx,
                            output, output_size, output_length
        );
    ...
    psa_cipher_operation_t *operation )
{
    ...
    switch( operation->id )
    {
        ...
        status = stm32_hal_transparent_cipher_abort
            (&operation->ctx.stm32_hal_driver_ctx
        );
    ...
    size_t *hash_length)
{
    ...
    status = stm32_hal_transparent_hash_compute
        (alg, input, input_length, hash, hash_size, hash_length
    );
    ...
    psa_algorithm_t alg )
{
    ...
    status = mbedtls_test_transparent_hash_setup
        (&operation->ctx.mbedtls_test_driver_ctx, alg
    );
    if(status == PSA_SUCCESS)
        operation->id = MBEDTLS_TEST_TRANSPARENT_DRIVER_ID;
    ...
    status = stm32_hal_transparent_hash_setup
        (&operation->ctx.stm32_hal_driver_ctx, alg
    );
    if(status == PSA_SUCCESS)
        operation->id = STM32_HAL_TRANSPARENT_DRIVER_ID;
    ...
    psa_hash_operation_t *target_operation )
{
    ...
    switch( source_operation->id )
    {
        ...
        status = mbedtls_test_transparent_hash_clone
            (&source_operation->ctx.mbedtls_test_driver_ctx, &target_operation->ctx.mbedtls_test_driver_ctx
        );
        ...
        target_operation->id = STM32_HAL_TRANSPARENT_DRIVER_ID;

        status = stm32_hal_transparent_hash_clone
            (&source_operation->ctx.stm32_hal_driver_ctx, &target_operation->ctx.stm32_hal_driver_ctx
        );
    ...
    size_t input_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_hash_update
            (&operation->ctx.mbedtls_test_driver_ctx, input, input_length
        );
        ...
        status = stm32_hal_transparent_hash_update
            (&operation->ctx.stm32_hal_driver_ctx, input, input_length
        );
    ...
    size_t *hash_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_hash_finish
            (&operation->ctx.mbedtls_test_driver_ctx, hash, hash_size, hash_length
        );
        ...
        status = stm32_hal_transparent_hash_finish
            (&operation->ctx.stm32_hal_driver_ctx, hash, hash_size, hash_length
        );
    ...
    psa_hash_operation_t *operation )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_hash_abort
            (&operation->ctx.mbedtls_test_driver_ctx
        );
        ...
        status = stm32_hal_transparent_hash_abort
            (&operation->ctx.stm32_hal_driver_ctx
        );
    ...
    uint8_t *ciphertext, size_t ciphertext_size, size_t *ciphertext_length )
{
    ...
    status = stm32_hal_transparent_aead_encrypt
        (attributes, key_buffer, key_buffer_size, alg, nonce, nonce_length,
                        additional_data, additional_data_length, plaintext, plaintext_length,
                        ciphertext, ciphertext_size, ciphertext_length
    );
    ...
    uint8_t *plaintext, size_t plaintext_size, size_t *plaintext_length )
{
    ...
    status = stm32_hal_transparent_aead_decrypt
        (attributes, key_buffer, key_buffer_size, alg, nonce, nonce_length,
                        additional_data, additional_data_length, ciphertext, ciphertext_length,
                        plaintext, plaintext_size, plaintext_length
    );
   ...
   psa_algorithm_t alg )
{
    ...
    switch( location )
    {
        ...
        status = stm32_hal_transparent_aead_encrypt_setup
            (&operation->ctx.stm32_hal_driver_ctx,
                            attributes, key_buffer, key_buffer_size, alg
        );
        if(status == PSA_SUCCESS)
   ...
   psa_algorithm_t alg )
{
    ...
    switch( location )
    {
        ...
        status = stm32_hal_transparent_aead_decrypt_setup
            (&operation->ctx.stm32_hal_driver_ctx,
                            attributes, key_buffer, key_buffer_size, alg
        );
        if(status == PSA_SUCCESS)
   ...
   size_t nonce_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_aead_set_nonce
            (&operation->ctx.mbedtls_test_driver_ctx,
                            nonce, nonce_length
        );
        ...
        status = stm32_hal_transparent_aead_set_nonce
            (&operation->ctx.stm32_hal_driver_ctx,
                            nonce, nonce_length
        );
   ...
   size_t plaintext_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_aead_set_lengths
            (&operation->ctx.mbedtls_test_driver_ctx,
        ...
        status = stm32_hal_transparent_aead_set_lengths
            (&operation->ctx.stm32_hal_driver_ctx,
                            ad_length, plaintext_length
        );
   ...
   size_t input_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_aead_update_ad
            (&operation->ctx.mbedtls_test_driver_ctx,
                            input, input_length
        );
        ...
        status = stm32_hal_transparent_aead_update_ad
            (&operation->ctx.stm32_hal_driver_ctx,
                            input, input_length
        );
   ...
   size_t *output_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_aead_update
            (&operation->ctx.mbedtls_test_driver_ctx,
                            input, input_length, output, output_size, output_length
        );
        ...
        status = stm32_hal_transparent_aead_update
            (&operation->ctx.stm32_hal_driver_ctx,
                            input, input_length, output, output_size, output_length
        );
   ...
   size_t *tag_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_aead_finish
            (&operation->ctx.mbedtls_test_driver_ctx,
        ...
        status = stm32_hal_transparent_aead_finish
            (&operation->ctx.stm32_hal_driver_ctx,
                            ciphertext, ciphertext_size, ciphertext_length, tag, tag_size, tag_length
        );
   ...
   size_t tag_length )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_aead_verify
            (&operation->ctx.mbedtls_test_driver_ctx,
                            plaintext, plaintext_size, plaintext_length, tag, tag_length
        );
        ...
        status = stm32_hal_transparent_aead_verify
            (&operation->ctx.stm32_hal_driver_ctx,
                            plaintext, plaintext_size, plaintext_length, tag, tag_length
        );
   ...
   psa_aead_operation_t *operation )
{
    ...
    switch( operation->id )
    {
        ...
        status = mbedtls_test_transparent_aead_abort
            (&operation->ctx.mbedtls_test_driver_ctx
        );
        ...
        status = stm32_hal_transparent_aead_abort
            (&operation->ctx.stm32_hal_driver_ctx
        );
