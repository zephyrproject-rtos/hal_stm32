...
/**
  * @brief Handle LCD interrupt request.
  * @param hlcd LCD handle.
  * @retval None
  */
void HAL_LCD_IRQHandler(LCD_HandleTypeDef *hlcd)
{
  ...
  uint32_t status = READ_REG(hlcd->Instance->SR);

  /* Handle Update Display Done (UDD) */
  if (status & LCD_SR_UDD)
  {
    /* Clear UDD flag by writing 1 to UDDC in LCD_CLR */
    __HAL_LCD_CLEAR_FLAG(hlcd, LCD_CLR_UDDC);

    /* User callback or processing */
    HAL_LCD_UpdateDisplayDoneCallback(hlcd);
  }

  /* Handle Start Of Frame (SOF) */
  if (status & LCD_SR_SOF)
  {
    /* Clear SOF flag by writing 1 to SOFC in LCD_CLR */
    __HAL_LCD_CLEAR_FLAG(hlcd, LCD_CLR_SOFC);

    /* User callback or processing */
    HAL_LCD_StartOfFrameCallback(hlcd);
  }
}

/**
  * @brief Display update done callback.
  * @param hlcd LCD handle.
  * @retval None
  */
__weak void HAL_LCD_UpdateDisplayDoneCallback(LCD_HandleTypeDef *hlcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hlcd);

  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_LCD_UpdateDisplayDoneCallback can be implemented in the user file.
   */
}
...
/**
  ...
  * @retval None
  */
__weak void HAL_LCD_StartOfFrameCallback(LCD_HandleTypeDef *hlcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hlcd);

  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_LCD_StartOfFrameCallback can be implemented in the user file.
   */
}
/**
  * @}
