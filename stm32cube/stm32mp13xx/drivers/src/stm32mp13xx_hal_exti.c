...
#define EXTI_CONFIG_OFFSET                  0x08U   /* 0x20: offset between CPU Rising/Falling
...
HAL_StatusTypeDef HAL_EXTI_SetConfigLine(EXTI_HandleTypeDef *hexti, EXTI_ConfigTypeDef *pExtiConfig)
{
  ...
  /* Set Interrupt And Event Mask for Core if configuration given into parameter mode */
  if ((pExtiConfig->Mode) != 0x00U)
  {
    ...
    *regaddr = regval;
    ...
    assert_param(((pExtiConfig->Line & EXTI_EVENT) == EXTI_EVENT) || ((pExtiConfig->Mode &\
                                                                       EXTI_MODE_EVENT) != EXTI_MODE_EVENT));
  ...
  if ((hexti->Line & EXTI_CONFIG) != 0x00U)
  {
    ...
    *regaddr = regval;
    ...
    if ((hexti->Line & EXTI_GPIO) == EXTI_GPIO)
    {
      ...
      regval = EXTI->EXTICR[(linepos >> 2U) & 0x03UL];
      regval &= ~(EXTI_EXTICR1_EXTI0 << (EXTI_EXTICR1_EXTI1_Pos * (linepos & 0x03UL)));
      EXTI->EXTICR[(linepos >> 2U) & 0x03UL] = regval;
    }
  }
