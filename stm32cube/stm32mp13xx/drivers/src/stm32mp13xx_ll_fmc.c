  ...
  [..] All external memories share the addresses, data and control signals with the controller.
       ...
       The main features of the FMC controller are the following:
        (+) Interface with static-memory mapped devices including:
           ...
           (++) Two banks of NAND Flash memory with ECC hardware (Hamming, BCH4 or BCH8 algorithm)
                to check up to 8 Kbytes of data
                                    ...
                                    FMC_NORSRAM_InitTypeDef *Init)
{
  ...
  {
    MODIFY_REG(Device->BTCR[FMC_NORSRAM_BANK1], FMC_BCRx_CCLKEN, Init->ContinuousClock);
  }
                                          ...
                                          FMC_NORSRAM_TimingTypeDef *Timing, uint32_t Bank)
{
  ...
  if (HAL_IS_BIT_SET(Device->BTCR[FMC_NORSRAM_BANK1], FMC_BCRx_CCLKEN))
  {
  ...
  *
  ...
  [..]
    ...
    (+) FMC NAND bank enable/disable ECC correction feature using the functions
        ...
        FMC_NAND_ECC_EnableHamming()/FMC_NAND_ECC_EnableBCH()
...
HAL_StatusTypeDef FMC_NAND_Init(FMC_NAND_TypeDef *Device, FMC_NAND_InitTypeDef *Init)
{
  ...
  assert_param(IS_FMC_ECC_ALGORITHM(Init->EccAlgorithm));
  assert_param(IS_FMC_BCH_MODE(Init->BCHMode));
  assert_param(IS_FMC_ECC_SECTOR_SIZE(Init->EccSectorSize));
...
HAL_StatusTypeDef FMC_NAND_DeInit(FMC_NAND_TypeDef *Device, uint32_t Bank)
{
  ...
  WRITE_REG(Device->SR,   0x00000053U);
...
HAL_StatusTypeDef FMC_NAND_ECC_Enable(FMC_NAND_TypeDef *Device, uint32_t Bank)
{
  ...
  /* Enable ECC feature */
  __FMC_NAND_ENABLE_ECC(Device, Bank);
...
HAL_StatusTypeDef FMC_NAND_ECC_Disable(FMC_NAND_TypeDef *Device, uint32_t Bank)
{
  ...
  /* Disable ECC feature */
  __FMC_NAND_DISABLE_ECC(Device, Bank);
                                  ...
                                  uint32_t Timeout)
{
  ...
  /* Wait until ECC is ready */
  while (__FMC_NAND_IS_ECC_HAMMING_READY(Device) == RESET)
  {
...
HAL_StatusTypeDef FMC_NAND_ECC_SetAlgorithm(FMC_NAND_TypeDef *Device, uint32_t Algorithm)
{
  ...
  __FMC_NAND_SET_ECC_ALGORITHM(Device, Algorithm);

  return HAL_OK;
}

/**
  * @brief  Sets dynamically the ECC access mode.
  * @param  Device Pointer to NAND device instance
  * @param  Mode Access mode to use for the sequencer and BCH ECC:
  *               FMC_NAND_ECC_ACCESS_MODE_READ or FMC_NAND_ECC_ACCESS_MODE_WRITE
  * @retval HAL status
  */
HAL_StatusTypeDef  FMC_NAND_ECC_SetAccessMode(FMC_NAND_TypeDef *Device, uint32_t Mode)
{
  ...
  __FMC_NAND_SET_ECC_ACCESS_MODE(Device, Mode);
...
/**
  ...
  * @param  Device Pointer to NAND device instance
  * @param  Mode BCH ECC mode: FMC_NAND_BCH_MODE_4BIT or FMC_NAND_BCH_MODE_8BIT
  * @retval HAL status
  */
HAL_StatusTypeDef  FMC_NAND_ECC_SetBCHMode(FMC_NAND_TypeDef *Device, uint32_t Mode)
{
  ...
  /* Change the BCH mode */
  __FMC_NAND_SET_BCH_MODE(Device, Mode);
...
HAL_StatusTypeDef FMC_NAND_ECC_GetBCH(FMC_NAND_TypeDef *Device, uint32_t *ECCval, uint32_t Timeout)
{
  ...
  /* Wait until ECC is ready */
  while (__FMC_NAND_IS_ECC_BCH_ENCODER_READY(Device) == RESET)
  {
    /* Check for the Timeout */
    if (Timeout != HAL_MAX_DELAY)
    {
  ...
  ECCval[0] = Device->BCHPBR1;
  ECCval[1] = Device->BCHPBR2;
  ECCval[2] = Device->BCHPBR3;
  ECCval[3] = Device->BCHPBR4;

  return HAL_OK;
}

/**
  * @brief  Gets the BCH result registers after reading a sector.
  * @param  Device Pointer to NAND device instance
  * @param  BCHDSRx Pointer to an array of 5 values containing the five BCHDSRx registers
  * @param  Timeout Timeout wait value
  * @retval HAL status
  */
                                                     ...
                                                     uint32_t Timeout)
{
  ...
  /* Wait until ECC is ready */
  while (__FMC_NAND_IS_ECC_BCH_DECODER_READY(Device) == RESET)
  {
    /* Check for the Timeout */
    if (Timeout != HAL_MAX_DELAY)
    {
  ...
  BCHDSRx[2] = Device->BCHDSR2;
  BCHDSRx[3] = Device->BCHDSR3;
  BCHDSRx[4] = Device->BCHDSR4;
...
/**
  ...
  * - Otherwise ErrorCount is set to the number of errors found (up to 8), and Positions if filled with the error
  *   positions
  * @param  BCHDSRx Pointer to an array of 5 values containing the five BCHDSRx registers
  ...
  */
void FMC_NAND_ECC_DecodeBCHResult(const uint32_t *BCHDSRx, uint16_t *Positions, int32_t *ErrorCount)
{
  uint32_t BCHDSR = BCHDSRx[0];
  if ((BCHDSR & FMC_BCHDSR0_DEF) == 0U)
  {
    /* There is no error */
    *ErrorCount = 0;
  }
  else if ((BCHDSR & FMC_BCHDSR0_DUE) != 0U)
  {
    /* There are too many errors, they cannot be corrected */
    *ErrorCount = -1;
  }
  else
  {
    /* There are some correctable errors */
    BCHDSR = (BCHDSR & FMC_BCHDSR0_DEN_Msk) >> FMC_BCHDSR0_DEN_Pos;
    *ErrorCount = (int32_t)BCHDSR;
    BCHDSR = BCHDSRx[1];
    Positions[0] = (uint16_t)(BCHDSR & 0x1fffU);
    Positions[1] = (uint16_t)((BCHDSR >> 16) & 0x1fffU);
    BCHDSR = BCHDSRx[2];
    Positions[2] = (uint16_t)(BCHDSR & 0x1fffU);
    Positions[3] = (uint16_t)((BCHDSR >> 16) & 0x1fffU);
    BCHDSR = BCHDSRx[3];
    Positions[4] = (uint16_t)(BCHDSR & 0x1fffU);
    Positions[5] = (uint16_t)((BCHDSR >> 16) & 0x1fffU);
    BCHDSR = BCHDSRx[4];
    Positions[6] = (uint16_t)(BCHDSR & 0x1fffU);
    Positions[7] = (uint16_t)((BCHDSR >> 16) & 0x1fffU);
  }
}
