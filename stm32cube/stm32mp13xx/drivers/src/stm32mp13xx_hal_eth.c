...
HAL_StatusTypeDef HAL_ETH_Init(ETH_HandleTypeDef *heth)
{
  ...
  if (heth->Init.MediaInterface == HAL_ETH_MII_MODE)
  {
    ...
    {
      HAL_SYSCFG_ETHInterfaceSelect(SYSCFG_ETH1_MII);
    }
    else
    {
      HAL_SYSCFG_ETHInterfaceSelect(SYSCFG_ETH2_MII);
    }
  }
  else if (heth->Init.MediaInterface == HAL_ETH_RGMII_MODE)
  {
    if (heth->Instance == ETH)
    {
      HAL_SYSCFG_ETHInterfaceSelect(SYSCFG_ETH1_RGMII);
    }
    else
    {
      HAL_SYSCFG_ETHInterfaceSelect(SYSCFG_ETH2_RGMII);
    }
  }
  ...
  WRITE_REG(heth->Instance->MAC1USTCR, (((uint32_t)HAL_RCC_GetACLKFreq() / ETH_MAC_US_TICK) - 1U));
...
void HAL_ETH_IRQHandler(ETH_HandleTypeDef *heth)
{
  ...
  uint32_t exti_flag = READ_REG(EXTI_C1->IMR3);
