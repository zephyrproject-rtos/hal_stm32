/**
  ...
  ******************************************************************************
  ...
  (@) Sections "ADC peripheral features" and "How to use this driver" are
      available in file of generic functions "stm32mp13xx_hal_adc.c".
  [..]
  @endverbatim
  ******************************************************************************
  */
...
/* source PLL4_Q 4MHz, ADC clock prescaler 256), CPU frequency 650MHz.        */
/* Calibration time max = 16384 / fADC (refer to datasheet)                   */
/*                      = 682000000 CPU cycles                                */
#define ADC_CALIBRATION_TIMEOUT         (682000000U)   /*!< ADC calibration time-out value */
...
#if defined(ADC_MULTIMODE_SUPPORT)
/**
  * @brief  Stop DMA-based multimode ADC conversion, disable ADC DMA transfer, disable ADC peripheral if no
  *         injected conversion is on-going.
  * @note   Multimode is kept enabled after this function. Multimode DMA bits
  *         (MDMA and DMACFG bits of common CCR register) are maintained. To disable
  *         multimode (set with HAL_ADCEx_MultiModeConfigChannel()), ADC must be
  *         reinitialized using HAL_ADC_Init() or HAL_ADC_DeInit(), or the user can
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef *hadc, ADC_InjectionConfTypeDef *pConfigInjected)
{
      ...
      (pConfigInjected->InjectedNbrOfConversion == 1U))
  {
    ...
    if (pConfigInjected->InjectedRank == ADC_INJECTED_RANK_1)
    {
      ...
      {
        tmp_JSQR_ContextQueueBeingBuilt = (ADC_JSQR_RK(pConfigInjected->InjectedChannel, ADC_INJECTED_RANK_1)
  ...
  else
  {
    ...
    if (hadc->InjectionConfig.ChannelCount == 0U)
    {
      ...
      if (pConfigInjected->ExternalTrigInjecConv != ADC_INJECTED_SOFTWARE_START)
      {
        tmp_JSQR_ContextQueueBeingBuilt = ((pConfigInjected->InjectedNbrOfConversion - 1U)
                                           | (pConfigInjected->ExternalTrigInjecConv & ADC_JSQR_JEXTSEL)
                                           | pConfigInjected->ExternalTrigInjecConvEdge
                                          );
      }
    ...
    tmp_JSQR_ContextQueueBeingBuilt &= ~ADC_JSQR_RK(ADC_SQR3_SQ10, pConfigInjected->InjectedRank);

    /* Set the JSQx bits for the selected rank */
    tmp_JSQR_ContextQueueBeingBuilt |= ADC_JSQR_RK(pConfigInjected->InjectedChannel, pConfigInjected->InjectedRank);
  ...
  if (LL_ADC_IsEnabled(hadc->Instance) == 0UL)
  {
      ...
      LL_ADC_SetChannelSamplingTime(hadc->Instance,
                                    (uint32_t)(__LL_ADC_DECIMAL_NB_TO_CHANNEL(
                                                 (__LL_ADC_CHANNEL_TO_DECIMAL_NB(
                                                  ...
                                                  + 1UL) & 0x1FUL)), pConfigInjected->InjectedSamplingTime);
    }
    ...
    if (__LL_ADC_IS_CHANNEL_INTERNAL(pConfigInjected->InjectedChannel))
    {
      ...
      if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(__LL_ADC_COMMON_INSTANCE(hadc->Instance)) == 0UL)
      {
        ...
        else if ((pConfigInjected->InjectedChannel == ADC_CHANNEL_VCORE)
