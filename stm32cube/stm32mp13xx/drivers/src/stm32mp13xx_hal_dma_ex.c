  ...
  ******************************************************************************
  ...
  The DMA Extension HAL driver can be used as follows:
   ...
   (+) To handle the DMAMUX Interrupts, the function  HAL_DMAEx_MUX_IRQHandler should be called from
       ...
       HAL_DMA_MUX_IRQHandler should be called with, as parameter, the appropriate
       DMA handle as many as used DMAs in the user project
...
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxSync(DMA_HandleTypeDef *hdma, HAL_DMA_MuxSyncConfigTypeDef *pSyncConfig)
{
  ...
  if (pSyncConfig->SyncEnable == ENABLE)
  {
    ...
    if (IS_DMA3_INSTANCE(hdma))
    {
      assert_param(IS_DMAMUX2_SYNC_SIGNAL_ID(pSyncConfig->SyncSignalID));
    }
    else
    {
      assert_param(IS_DMAMUX1_SYNC_SIGNAL_ID(pSyncConfig->SyncSignalID));
    }
                                                      ...
                                                      HAL_DMA_MuxRequestGeneratorConfigTypeDef *pRequestGeneratorConfig)
{
  ...
  if (IS_DMA3_INSTANCE(hdma))
  {
    assert_param(IS_DMAMUX2_REQUEST_GEN_SIGNAL_ID(pRequestGeneratorConfig->SignalID));
  }
  else
  {
    assert_param(IS_DMAMUX1_REQUEST_GEN_SIGNAL_ID(pRequestGeneratorConfig->SignalID));
  }
