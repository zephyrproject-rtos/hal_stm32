...
#define IS_LL_DMA3_REQUEST(REQUEST)             (((REQUEST)   <= LL_DMAMUX2_REQ_SEC_TIM17_UP))
...
uint32_t LL_DMA_DeInit(DMA_TypeDef *DMAx, uint32_t Stream)
{
  ...
  if (Stream == LL_DMA_STREAM_ALL)
  {
    ...
    {
      /* Force reset of DMA clock */
      LL_AHB2_GRP1_ForceReset(LL_AHB2_GRP1_PERIPH_DMA3);

      /* Release reset of DMA clock */
      LL_AHB2_GRP1_ReleaseReset(LL_AHB2_GRP1_PERIPH_DMA3);
    }
    else
    {
      status = ERROR;
...
uint32_t LL_DMA_Init(DMA_TypeDef *DMAx, uint32_t Stream, LL_DMA_InitTypeDef *DMA_InitStruct)
{
  ...
  if (DMAx == DMA3)
  {
    assert_param(IS_LL_DMA3_REQUEST(DMA_InitStruct->PeriphRequest));
  }
