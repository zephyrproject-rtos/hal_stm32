...
================================================================================
  ...
  [..]
      The STM32MP13xx device family integrate one DTS sensor interface :
...
#define DTS_CFGR1_RESET_VALUE (0UL)
...
HAL_StatusTypeDef HAL_DTS_Init(DTS_HandleTypeDef *hdts)
{
  ...
  if (__HAL_DTS_GET_TRIGGER(hdts) == DTS_TRIGGER_HW_NONE_CONT)
  {
    MODIFY_REG(hdts->Instance->CFGR1, DTS_CFGR1_TS1_INTRIG_SEL,  DTS_TRIGGER_HW_NONE);
  }
  else
  {
  ...
  MODIFY_REG(hdts->Instance->ITR1, DTS_ITR1_TS1_HITTHD, (hdts->Init.HighInterruptThreshold << DTS_ITR1_TS1_HITTHD_Pos));
  MODIFY_REG(hdts->Instance->ITR1, DTS_ITR1_TS1_LITTHD, hdts->Init.LowInterruptThreshold);
...
HAL_StatusTypeDef HAL_DTS_Start(DTS_HandleTypeDef *hdts)
{
  ...
  if (hdts->State == HAL_DTS_STATE_READY)
  {
    ...
    while (__HAL_DTS_GET_FLAG(hdts, DTS_FLAG_TS1_RDY) == 0U)
    {
      if ((HAL_GetTick() - tickstart) > DTS_DELAY_STARTUP)
      {
        return HAL_TIMEOUT;
      }
    }
...
HAL_StatusTypeDef HAL_DTS_Start_IT(DTS_HandleTypeDef *hdts)
{
  ...
  if (hdts->State == HAL_DTS_STATE_READY)
  {
    ...
    if ((hdts->Init.InterruptSource & DTS_IT_ASYNC) != 0U)
    {
      if (__HAL_DTS_GET_REFCLK(hdts, DTS_REFCLKSEL_LSE) == 0U)
      {
        return HAL_ERROR;
      }
      else
...
HAL_StatusTypeDef HAL_DTS_GetTemperature(DTS_HandleTypeDef *hdts, int32_t *Temperature)
{
  ...
  uint32_t t0_freq;
  uint32_t ramp_coeff;
  uint32_t count_read = 0UL;
  uint32_t tmp_smp_time;
  ...
  if (hdts->State == HAL_DTS_STATE_READY)
  {
    ...
    if ((__HAL_DTS_GET_TRIGGER(hdts) == DTS_TRIGGER_HW_NONE)  &&
        (__HAL_DTS_GET_TRIGGER(hdts) != DTS_TRIGGER_HW_NONE_CONT))
    {
      /* Start measure */
      SET_BIT(hdts->Instance->CFGR1, DTS_CFGR1_TS1_START);
    ...
    /* Save the total number of samples (debug). */
    ...
    {
      freq_meas = LSE_VALUE * samples / tmp_smp_time ; /* On Hz */
    }
    else
    {
      freq_meas = ((uint32_t)HS_CLK_FREQ / samples) * tmp_smp_time;   /* On Hz */
    }
...
void HAL_DTS_IRQHandler(DTS_HandleTypeDef *hdts)
{
  ...
  if ((__HAL_DTS_GET_FLAG(hdts, DTS_FLAG_TS1_AITEF)) != 0U)
  {
  ...
  if ((__HAL_DTS_GET_FLAG(hdts, DTS_FLAG_TS1_AITLF)) != 0U)
  {
  ...
  if ((__HAL_DTS_GET_FLAG(hdts, DTS_FLAG_TS1_AITHF)) != 0U)
  {
  ...
  if ((__HAL_DTS_GET_FLAG(hdts, DTS_FLAG_TS1_ITEF)) != 0U)
  {
  ...
  if ((__HAL_DTS_GET_FLAG(hdts, DTS_FLAG_TS1_ITLF)) != 0U)
  {
  ...
  if ((__HAL_DTS_GET_FLAG(hdts, DTS_FLAG_TS1_ITHF)) != 0U)
  {
