...
static void HASH_MDMAXferCplt2(MDMA_HandleTypeDef *hdma);
static void HASH_MDMAXferCplt(MDMA_HandleTypeDef *hdma);
static void HASH_MDMAError(MDMA_HandleTypeDef *hdma);
...
/**
  ...
  * @retval None
  */
void HAL_HASH_ContextSaving(HASH_HandleTypeDef *hhash, uint32_t *pMemBuffer)
{
  ...
  *(uint32_t *)(mem_ptr) = READ_REG(HASH->IMR);
  mem_ptr += 4U;

  /* Save STR register content */
  *(uint32_t *)(mem_ptr) = READ_REG(HASH->STR);
  mem_ptr += 4U;

  /* Save CR register content */
  *(uint32_t *)(mem_ptr) = READ_REG(HASH->CR);
  mem_ptr += 4U;
  /* Save SHA3CFG register content */
  *(uint32_t *)(mem_ptr) = READ_REG(HASH->SHA3CFGR);
  mem_ptr += 4U;
...
/**
  ...
  * @retval None
  */
void HAL_HASH_ContextRestoring(HASH_HandleTypeDef *hhash, uint32_t *pMemBuffer)
{
  uint32_t mem_ptr = (uint32_t)pMemBuffer;
  uint32_t csr_ptr = (uint32_t)HASH->CSR;
  uint32_t i;
  ...
  /* Restore SHA3CFG register content */
  WRITE_REG(HASH->SHA3CFGR, (*(uint32_t *)(mem_ptr)));
  mem_ptr += 4U;

  /* Reset the HASH processor before restoring the Context Swap Registers (CSR) */
  __HAL_HASH_INIT();
...
HAL_StatusTypeDef HAL_HASH_DMAFeed_ProcessSuspend(HASH_HandleTypeDef *hhash)
{
  ...
  else
  {
    ...
    if (__HAL_HASH_GET_FLAG(HASH_FLAG_DCIS) != 0U)
    {
      return HAL_ERROR;
    }
    ...
    */
...
#ifdef HAL_MDMA_MODULE_ENABLED
    if (hhash->hmdmain != NULL)
    {
      if (HAL_MDMA_Abort(hhash->hmdmain) == HAL_OK)
      {
        return HAL_ERROR;
      }
    }
    ...
    if (hhash->hmdmain != NULL)
    {
      tmp_remaining_DMATransferSize_inWords = (hhash->hmdmain->Instance->CBNDTR & MDMA_CBNDTR_BNDT);
    }
#endif /* HAL_MDMA_MODULE_ENABLED */
...
/**
  ...
  */
#ifdef HAL_MDMA_MODULE_ENABLED
static void HASH_MDMAXferCplt2(MDMA_HandleTypeDef *hmdma)
{
  HASH_HandleTypeDef *hhash = (HASH_HandleTypeDef *)((MDMA_HandleTypeDef *)hmdma)->Parent;
  uint32_t datasentsize;
  uint32_t remainsize;
  HAL_StatusTypeDef status;

  if ((HASH->HWCFGR & 0xFFU) >= 1U)
  {
    CLEAR_BIT(HASH->CR, HASH_CR_DMAE);
    /* Next DMA is sent from the DMA handler, dma instance must be reset */
    if (READ_BIT(HASH->CR, HASH_CR_MODE) != 0U)
    {
      hmdma->State = HAL_MDMA_STATE_BUSY; /* to avoid error in MDMA_Abort */
      status = HAL_MDMA_Abort(hmdma);
      if (status != HAL_OK)
      {
        hhash->State = HAL_HASH_STATE_ERROR;
      }
    }
    SET_BIT(HASH->CR, HASH_CR_DMAA);
    datasentsize = (((hhash->HashInCount % 16U) != 0U) ? \
                    ((hhash->HashInCount / 16U) * 16U) : (hhash->HashInCount - 16U));
    remainsize = hhash->HashInCount - datasentsize;
    status = HASH_WriteData(hhash, (hhash->pHashInBuffPtr) + datasentsize, remainsize);
    if (status != HAL_OK)
    {
      hhash->Status = status;
    }
    if (READ_BIT(HASH->CR, HASH_CR_MDMAT) == 0U)
    {
...
static void HASH_MDMAXferCplt(MDMA_HandleTypeDef *hmdma)
{
  HASH_HandleTypeDef *hhash = (HASH_HandleTypeDef *)((MDMA_HandleTypeDef *)hmdma)->Parent;
  uint32_t inputaddr = 0x0U;
  uint32_t buffersize = 0x0U;
  HAL_StatusTypeDef status = HAL_OK;
  ...
  if (hhash->State != HAL_HASH_STATE_SUSPENDED)
  {
    ...
    else
    {
      ...
      else if (hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_1)
      {
        ...
        /* Check whether or not digest calculation must be disabled
        * (in case of multi-buffer HMAC processing) */
      ...
      if (hhash->hmdmain != NULL)
      {
        hhash->hmdmain->XferCpltCallback = HASH_MDMAXferCplt;
        /* Enable the DMA In DMA Stream */
        status = HAL_MDMA_Start_IT(hhash->hmdmain, inputaddr, (uint32_t)&HASH->DIN,
                                   (((buffersize % 16U) != 0U)  ? ((buffersize / 16U) * 16U) : (buffersize - 16U)), 1U);
      }
...
static void HASH_DMAXferCplt(DMA_HandleTypeDef *hdma)
{
  ...
  if (hhash->State != HAL_HASH_STATE_SUSPENDED)
  {
    ...
    if (READ_BIT(HASH->CR, HASH_CR_DMAE) != 0U)
    {
      hhash->State = HAL_HASH_STATE_ERROR;
      return;
    }
...
/**
  ...
  */
#ifdef HAL_MDMA_MODULE_ENABLED
static void HASH_MDMAError(MDMA_HandleTypeDef *hmdma)
{
  HASH_HandleTypeDef *hhash = (HASH_HandleTypeDef *)((MDMA_HandleTypeDef *)hmdma)->Parent;
...
static HAL_StatusTypeDef HASH_WriteData(HASH_HandleTypeDef *hhash, uint8_t *pInBuffer, uint32_t Size)
{
  ...
  for (buffercounter = 0U; buffercounter < Size; buffercounter += 4U)
  {
    ...
    /* If a block is processed, enf of the processing  (occurs at each block+1 DIN writing) */
    if (HASH_WaitOnFlagUntilTimeout(hhash, HASH_FLAG_BUSY, SET, HASH_TIMEOUTVALUE) != HAL_OK)
    {
      return HAL_TIMEOUT;
    }

    if ((hhash->SuspendRequest == HAL_HASH_SUSPEND) && (__HAL_HASH_GET_FLAG(HASH_FLAG_DINIS) == SET))
    {
      /*if a  suspension is requested and if a partial hash is ready,
      function shall stop filling HASH buffer and save filled level value and exit*/

      /* Reset SuspendRequest and save remaining to write*/
      hhash->SuspendRequest = HAL_HASH_SUSPEND_NONE;
      ...
      if ((hhash->Phase == HAL_HASH_PHASE_PROCESS) || (hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_2))
      {
        ...
        hhash->HashInCount    =  Size - buffercounter;
      }
      else if ((hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_1) || (hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_3))
      {
        /* Save current reading and writing locations of Input and Output buffers */
        hhash->pHashKeyBuffPtr  = (uint8_t *)inputaddr;
        hhash->HashKeyCount     =  Size - buffercounter;
      }
      else
      {
    ...
    {
      /* Write input data 4 bytes at a time and save remaining to write */
...
static uint32_t HASH_Write_Block_Data(HASH_HandleTypeDef *hhash)
{
  ...
  uint32_t nbByteToWrite;

  /* Nb bytes to enter in HASH fifo to trig a partial HASH computation */
  uint32_t nbByteToWriteToTriggPartialHash  = ((HASH->SR & HASH_SR_NBWE_Msk) >> HASH_SR_NBWE_Pos) * 4U;

  inputaddr = (uint32_t)hhash->pHashInBuffPtr;
  if (hhash->HashInCount > nbByteToWriteToTriggPartialHash)
  {
    /*remaining bytes to hash greater than threshold for automatic computation*/
    nbByteToWrite = nbByteToWriteToTriggPartialHash;
    hhash->HashInCount -= nbByteToWrite;
    hhash->pHashInBuffPtr += nbByteToWrite;
  }
  else
  {
    nbByteToWrite = hhash->HashInCount;
    hhash->HashInCount = 0U;
    /* Disable Interrupts  for last block*/
    __HAL_HASH_DISABLE_IT(HASH_IT_DINI);
  }

  for (buffercounter = 0U; buffercounter < nbByteToWrite; buffercounter += 4U)
  {
    HASH->DIN = *(uint32_t *)inputaddr;
    inputaddr += 4U;
  }
  ...
  {
    /*When no automatic computation( i.e. size smaller to trigger threshold), request computing*/
    __HAL_HASH_START_DIGEST();
...
/**
  * @brief  Get the algo block size
  * @param  Algorithm HASH algorithm.
  * @retval block size in byte
  */
uint32_t HASH_GetBlockSize(uint32_t Algorithm)
{
  if (Algorithm == HASH_ALGOSELECTION_SHA1)         { return 64U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA224)       { return 64U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA256)       { return 64U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA384)       { return 128U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA512_224)   { return 128U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA512_256)   { return 128U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA512)       { return 128U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA3_224)     { return 144U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA3_256)     { return 136U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA3_384)     { return 104U; }
  if (Algorithm == HASH_ALGOSELECTION_SHA3_512)     { return 72U; }
  if (Algorithm == HASH_ALGOSELECTION_SHAKE_128)    { return 168U; }
  if (Algorithm == HASH_ALGOSELECTION_SHAKE_256)    { return 136U; }
  if (Algorithm == HASH_ALGOSELECTION_RAWSHAKE_128) { return 168U; }
  if (Algorithm == HASH_ALGOSELECTION_RAWSHAKE_256) { return 136U; }
  return 0U;
}
...
HAL_StatusTypeDef HASH_Start_DMA(HASH_HandleTypeDef *hhash, uint8_t *pInBuffer, uint32_t Size, uint32_t Algorithm)
{
  ...
  /* Make sure the input buffer size (in bytes) is a multiple of 16 when MDMAT bit is set
  ...
  if ((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
  {
    ...
    if (hhash->hmdmain != NULL)
    {
      ...
      hhash->hmdmain->XferErrorCallback = HASH_MDMAError;
      /* Enable the DMA In DMA Stream */
      if (READ_BIT(HASH->CR, HASH_CR_MDMAT) != 0U)
      {
        status = HAL_MDMA_Start_IT(hhash->hmdmain, inputaddr, (uint32_t)&HASH->DIN, inputSize, 1U);
      }
      else
      {
        status = HAL_MDMA_Start_IT(hhash->hmdmain, inputaddr, (uint32_t)&HASH->DIN,
                                   (((inputSize % 16U) != 0U) ? ((inputSize / 16U) * 16U) : (inputSize - 16U)), 1U);
      }
    }
#endif /* HAL_MDMA_MODULE_ENABLED */
    ...
    if (hhash->hdmain != NULL)
    {
      ...
      /* Store number of words already pushed to manage proper
         DMA processing suspension */
      hhash->NbWordsAlreadyPushed = HASH_NBW_PUSHED();

      /* Enable the DMA In DMA Stream */
      status = HAL_DMA_Start_IT(hhash->hdmain, inputaddr, (uint32_t)&HASH->DIN,
                                (((inputSize % 4U) != 0U) ? ((inputSize + 3U) / 4U) : (inputSize / 4U)));
                             ...
                             uint32_t Timeout, uint32_t Algorithm)
{
  ...
  if ((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
  {
    ...
    if (hhash->Phase == HAL_HASH_PHASE_READY)
    {
      blocksize = HASH_GetBlockSize(Algorithm);

      /* Check if key size is larger than blocksize, accordingly set LKEY and the other setting bits */
...
HAL_StatusTypeDef HMAC_Start_DMA(HASH_HandleTypeDef *hhash, uint8_t *pInBuffer, uint32_t Size, uint32_t Algorithm)
{
  ...
  HAL_StatusTypeDef status = HAL_OK;
  HAL_HASH_StateTypeDef State_tmp = hhash->State;
  /* Make sure the input buffer size (in bytes) is a multiple of 16 when digest calculation
