  ...
  ==============================================================================
    [..]
      ...
      (#)Initialize the CRYP low level resources by implementing the HAL_CRYP_MspInit():
         ...
         (##) In case of using MDMA to control data transfer (e.g. HAL_CRYP_Encrypt_MDMA())
             (+++) Enable the MDMAx interface clock using __RCC_MDMAx_CLK_ENABLE()
             (+++) Configure and enable two MDMA streams one for managing data transfer from
                 memory to peripheral (input stream) and another stream for managing data
                 transfer from peripheral to memory (output stream)
             (+++) Associate the initialized MDMA handle to the CRYP MDMA handle
                 using  __HAL_LINKMDMA()
             (+++) Configure the priority and enable the NVIC for the transfer complete
                 interrupt on the two MDMA Streams. The output stream should have higher
      ...
      (#)Three processing (encryption/decryption) functions are available:
         ...
         (##) MDMA mode: encryption and decryption APIs are not blocking functions
              i.e. the data transfer is ensured by MDMA,
              e.g. HAL_CRYP_Encrypt_MDMA & HAL_CRYP_Decrypt_MDMA
      ...
      (#)When the processing function is called at first time after HAL_CRYP_Init()
       ...
       (#)To process a single message with consecutive calls to HAL_CRYP_Encrypt() or HAL_CRYP_Decrypt()
          ...
          HAL_CRYP_Encrypt_MDMA()or HAL_CRYP_Decrypt_MDMA().
...
#define MDMA_SUSPENDED                   0x00000001U
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase_MDMA(CRYP_HandleTypeDef *hcryp);
static HAL_StatusTypeDef CRYP_AESGCM_Process_MDMA(CRYP_HandleTypeDef *hcryp);
static HAL_StatusTypeDef CRYP_AESGCM_Process_IT(CRYP_HandleTypeDef *hcryp);
static HAL_StatusTypeDef CRYP_AESGCM_Process(CRYP_HandleTypeDef *hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AESCCM_Process(CRYP_HandleTypeDef *hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AESCCM_Process_IT(CRYP_HandleTypeDef *hcryp);
static HAL_StatusTypeDef CRYP_AESCCM_Process_MDMA(CRYP_HandleTypeDef *hcryp);
...
static HAL_StatusTypeDef CRYP_AES_Decrypt_MDMA(CRYP_HandleTypeDef *hcryp);
...
/**
  * @brief  Request CRYP processing suspension when in MDMA mode.
  * @param  hcryp pointer to a CRYP_HandleTypeDef structure that contains
  *         the configuration information for CRYP module.
  * @note   Set the handle field SuspendRequest to the appropriate value so that
  *         the on-going CRYP processing is suspended as soon as the required
  *         conditions are met.
  * @note   HAL_CRYP_ProcessSuspend() can only be invoked when the processing is done
  *         in non-blocking MDMA mode.
  * @retval None
  */
HAL_StatusTypeDef HAL_CRYP_MDMAProcessSuspend(CRYP_HandleTypeDef *hcryp)
{
#if !defined(USE_HAL_SAES_ONLY) || (USE_HAL_SAES_ONLY == 1)
  uint32_t Timeout = CRYP_GENERAL_TIMEOUT;
#endif /* USE_HAL_SAES_ONLY */
  uint32_t tmp_remaining_MDMATransferSize_inWords;
  uint32_t tmp_initial_MDMATransferSize_inWords;

#if !defined(USE_HAL_CRYP_ONLY) || (USE_HAL_CRYP_ONLY == 1)
  if (IS_CRYP_INSTANCE(hcryp->Instance))
  {
    /* Stop the MDMA transfers to the IN FIFO by clearing to 0 the DIEN bit in
    ...
    /* Stop the MDMA transfers from the OUT FIFO by clearing to 0 the DOEN
  ...
  if (IS_SAES_INSTANCE(hcryp->Instance))
  {
    /* Stop the MDMA transfer to the IN FIFO by clearing the DMAINEN bit of the SAES_CR register */
    ...
    /* Stop the MDMA transfer from the OUT FIFO by clearing the DMAOUTEN bit of the SAES_CR register */
  ...
  tmp_remaining_MDMATransferSize_inWords = (uint32_t)(hcryp->hmdmain->FirstLinkedListNodeAddress + \
                                                      hcryp->hmdmain->LinkedListNodeCounter);
  tmp_remaining_MDMATransferSize_inWords = (uint32_t)(hcryp->hmdmain->FirstLinkedListNodeAddress - \
                                                      tmp_remaining_MDMATransferSize_inWords);

  /* Disable MDMA channels */
  /* Note that the Abort function will
    - Clear the transfer error flags
    - Unlock
    - Set the State
  */
  if (HAL_MDMA_Abort(hcryp->hmdmain) != HAL_OK)
  {
    return HAL_ERROR;
  }

  /* Compute how many words were supposed to be transferred by MDMA */
  /* hcryp->CrypInCount is in bytes, must be a multiple of 4 (no padding handled for ECB or CBC) */
  tmp_initial_MDMATransferSize_inWords = (hcryp->Size / 4U);

  /* Accordingly, update the input and output pointers that points at the next word to be
     transferred to the Peripheral by MDMA or read from the Peripheral by the MDMA */
  hcryp->pCrypInBuffPtr  += (tmp_initial_MDMATransferSize_inWords - tmp_remaining_MDMATransferSize_inWords) ;
  hcryp->pCrypOutBuffPtr += (tmp_initial_MDMATransferSize_inWords - tmp_remaining_MDMATransferSize_inWords) ;

  /* And store in CrypInCount the remaining size to transfer (in words) */
  hcryp->CrypInCount  = (uint16_t) tmp_remaining_MDMATransferSize_inWords;
  hcryp->CrypOutCount = (uint16_t) tmp_remaining_MDMATransferSize_inWords;
...
HAL_StatusTypeDef HAL_CRYP_Suspend(CRYP_HandleTypeDef *hcryp)
{
  ...
  /* Request suspension */
  /* Check whether the processing is in MDMA mode or not */
  ...
  {
    if (HAL_CRYP_MDMAProcessSuspend(hcryp) != HAL_OK)
    {
      return HAL_ERROR;
    }
...
HAL_StatusTypeDef HAL_CRYP_Resume(CRYP_HandleTypeDef *hcryp)
{
  ...
  else
  {
    ...
    if (hcryp->SuspendedProcessing == MDMA_SUSPENDED)
    {
      ...
      {
        if (HAL_CRYP_Encrypt_MDMA(hcryp, hcryp->pCrypInBuffPtr_saved, hcryp->CrypInCount_saved,
      ...
      {
        if (HAL_CRYP_Decrypt_MDMA(hcryp, hcryp->pCrypInBuffPtr_saved, hcryp->CrypInCount_saved,
                                  hcryp->pCrypOutBuffPtr_saved) != HAL_OK)
        {
          return HAL_ERROR;
        }
...
@verbatim
  ...
  ==============================================================================
    ...
    [..]  Three processing functions are available:
      ...
      (+) MDMA mode : HAL_CRYP_Encrypt_MDMA & HAL_CRYP_Decrypt_MDMA
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_CRYP_Encrypt_MDMA(CRYP_HandleTypeDef *hcryp, uint32_t *Input, uint16_t Size, uint32_t *Output)
{
  HAL_StatusTypeDef status = HAL_OK;
  uint32_t algo = 0U;
  ...
  if (hcryp->State == HAL_CRYP_STATE_READY)
  {
    ...
    switch (algo)
    {
      ...
      case CRYP_TDES_CBC:
        ...
        /* Start MDMA process transfer for DES/TDES */
        CRYP_SetMDMAConfig(hcryp, (uint32_t)(hcryp->pCrypInBuffPtr), (hcryp->Size / 4U),
      ...
      case CRYP_AES_CTR:
        ...
        /* Start MDMA process transfer for AES */
        CRYP_SetMDMAConfig(hcryp, (uint32_t)(hcryp->pCrypInBuffPtr), (hcryp->Size / 4U),
                           (uint32_t)(hcryp->pCrypOutBuffPtr));
        break;
      ...
      case CRYP_AES_GCM:
        ...
        /* AES GCM encryption */
        status = CRYP_AESGCM_Process_MDMA(hcryp);
        break;

      case CRYP_AES_CCM:

        /* AES CCM encryption */
        status = CRYP_AESCCM_Process_MDMA(hcryp);
        break;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_CRYP_Decrypt_MDMA(CRYP_HandleTypeDef *hcryp, uint32_t *Input, uint16_t Size, uint32_t *Output)
{
  uint32_t algo = 0U;
  HAL_StatusTypeDef status = HAL_ERROR;
  ...
  if (hcryp->State == HAL_CRYP_STATE_READY)
  {
    ...
    switch (algo)
    {
      ...
      case CRYP_AES_CTR:
        ...
        /* AES decryption */
        status = CRYP_AES_Decrypt_MDMA(hcryp);
        break;
...
/**
  * @brief  Decryption in ECB/CBC & CTR mode with AES Standard using MDMA mode
  * @param  hcryp: pointer to a CRYP_HandleTypeDef structure that contains
  *         the configuration information for CRYP module
  * @retval HAL status
  */
...
static void CRYP_MDMAInCplt(MDMA_HandleTypeDef *hmdma)
{
  ...
  if (IS_CRYP_INSTANCE(hcryp->Instance))
  {
    /* Disable the MDMA transfer for input FIFO request by resetting the DIEN bit
       in the MDMACR register */
  ...
  if (IS_SAES_INSTANCE(hcryp->Instance))
  {
    /* Disable the MDMA transfer for input FIFO request by resetting the DMAINEN bit
       in the CR register */
...
static void CRYP_MDMAOutCplt(MDMA_HandleTypeDef *hmdma)
{
  ...
  /* Disable the MDMA transfer for output FIFO */
#if !defined(USE_HAL_CRYP_ONLY) || (USE_HAL_CRYP_ONLY == 1)
  if (IS_CRYP_INSTANCE(hcryp->Instance))
  {
  ...
  if (IS_CRYP_INSTANCE(hcryp->Instance))
  {
    ...
    {
      /* set CrypInCount and CrypOutCount to exact number of word already computed via MDMA  */
...
static void CRYP_MDMAError(MDMA_HandleTypeDef *hmdma)
{
  ...
  /* MDMA error code field */
  hcryp->ErrorCode |= HAL_CRYP_ERROR_MDMA;
...
/**
  ...
  * @param  BufferSize: size of the input buffer, must be a multiple of 16.
  * @param  outputaddr: address of the output buffer
  * @retval None
  */
static void CRYP_SetMDMAConfig(CRYP_HandleTypeDef *hcryp, uint32_t inputaddr, uint16_t BufferSize,
                               uint32_t outputaddr)
{

  /* Set the CRYP MDMA transfer complete callback */
  hcryp->hmdmain->XferCpltCallback = CRYP_MDMAInCplt;

  /* Set the MDMA input error callback */
  hcryp->hmdmain->XferErrorCallback = CRYP_MDMAError;

  /* Set the CRYP MDMA transfer complete callback */
  hcryp->hmdmaout->XferCpltCallback = CRYP_MDMAOutCplt;

  /* Set the MDMA output error callback */
  ...
  if (IS_CRYP_INSTANCE(hcryp->Instance))
  {
    /* Enable the input MDMA Stream */
    if (HAL_MDMA_Start_IT(hcryp->hmdmain, inputaddr, (uint32_t) &((CRYP_TypeDef *)(hcryp->Instance))->DIN,
                          ((uint32_t)BufferSize * 4U), 1U) != HAL_OK)
    {
      /* MDMA error code field */
      hcryp->ErrorCode |= HAL_CRYP_ERROR_MDMA;
    ...
    /* Enable the output MDMA Stream */
    if (HAL_MDMA_Start_IT(hcryp->hmdmaout, (uint32_t) &((CRYP_TypeDef *)(hcryp->Instance))->DOUT, \
                          outputaddr, ((uint32_t)BufferSize * 4U), 1U) != HAL_OK)
    {
      /* MDMA error code field */
      hcryp->ErrorCode |= HAL_CRYP_ERROR_MDMA;
  ...
  if (IS_SAES_INSTANCE(hcryp->Instance))
  {
    /*  Once SAES_CR register enables MDMDA reading from SAES it initiates a MDMDA request during Input/Output
     *  phase each time it requires to read a 128-bit block (quadruple word) to the SAES_DINR register.
     *  For each word MDMA peripheral must process a single read/write.
    **/
    /* Enable the input MDMA Stream */
    if (HAL_MDMA_Start_IT(hcryp->hmdmain, inputaddr, (uint32_t) &((SAES_TypeDef *)(hcryp->Instance))->DINR, \
                          sizeof(uint32_t), ((uint32_t)BufferSize * 4U) / sizeof(uint32_t)) != HAL_OK)
    {
      /* MDMA error code field */
      hcryp->ErrorCode |= HAL_CRYP_ERROR_MDMA;
    ...
    /* Enable the output MDMA Stream */
    if (HAL_MDMA_Start_IT(hcryp->hmdmaout, (uint32_t) &((SAES_TypeDef *)(hcryp->Instance))->DOUTR, outputaddr, \
                          sizeof(uint32_t), ((uint32_t)BufferSize * 4U) / sizeof(uint32_t)) != HAL_OK)
    {
      /* MDMA error code field */
      hcryp->ErrorCode |= HAL_CRYP_ERROR_MDMA;
...
/**
  * @brief  Encryption/Decryption process in AES GCM mode and prepare the authentication TAG using MDMA
  * @param  hcryp: pointer to a CRYP_HandleTypeDef structure that contains
  *         the configuration information for CRYP module
  * @retval HAL status
  */
static HAL_StatusTypeDef CRYP_AESGCM_Process_MDMA(CRYP_HandleTypeDef *hcryp)
{
  ...
  if (DoKeyIVConfig == 1U)
  {
    ...
    if (CRYP_GCMCCM_SetHeaderPhase_MDMA(hcryp) != HAL_OK)
    {
      return HAL_ERROR;
    }
...
/**
  * @brief  AES CCM encryption/decryption process in MDMA mode
  * @param  hcryp: pointer to a CRYP_HandleTypeDef structure that contains
  *         the configuration information for CRYP module
  * @retval HAL status
  */
...
/**
  * @brief  Sets the header phase when using MDMA in process
  * @param  hcryp: pointer to a CRYP_HandleTypeDef structure that contains
  *         the configuration information for CRYP module(Header & HeaderSize)
  * @retval None
  */
