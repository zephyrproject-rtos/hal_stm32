  ...
  ******************************************************************************
  ...
  ==============================================================================
    ...
    (#) Optionally, in case of usage of ADC with interruptions:
         (++) Configure the GIC for ADC
              using function IRQ_Enable(ADCx_IRQn)
         (++) Insert the ADC interruption handler function HAL_ADC_IRQHandler()
              into the function of corresponding ADC interruption vector
              ADCx_IRQHandler().

    (#) Optionally, in case of usage of DMA:
         (++) Configure the DMA (DMA stream, mode normal or circular, ...)
              using function HAL_DMA_Init().
         (++) Configure the GIC for DMA
              using function IRQ_Enable(DMAx_Streamx_IRQn)
         (++) Insert the ADC interruption handler function HAL_ADC_IRQHandler()
    ...
    (#) Optionally, in case of usage of ADC with interruptions:
         (++) Disable the GIC for ADC
              using function IRQ_Disable(ADCx_IRQn)

    (#) Optionally, in case of usage of DMA:
         (++) Deinitialize the DMA
              using function HAL_DMA_Init().
         (++) Disable the GIC for DMA
              using function IRQ_Disable(DMAx_Streamx_IRQn)
...
#define ADC_CFGR2_FIELDS  ((uint32_t)(ADC_CFGR2_ROVSE |\
                                      ADC_CFGR2_OVSS | ADC_CFGR2_TROVS |\
                                      ADC_CFGR2_ROVSM))     /*!< ADC_CFGR2 fields of parameters that can be
                                                                 updated when no conversion (neither regular
...
HAL_StatusTypeDef HAL_ADC_DeInit(ADC_HandleTypeDef *hadc)
{
  ...
  if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(__LL_ADC_COMMON_INSTANCE(hadc->Instance)) == 0UL)
  {
    ...
    */
    ...
    /* DeInit the low level hardware: RCC clock, NVIC or GIC */
...
HAL_StatusTypeDef HAL_ADC_PollForConversion(ADC_HandleTypeDef *hadc, uint32_t Timeout)
{
  ...
  /* Retrieve handle ADC CFGR register */
  tmp_cfgr = READ_REG(hadc->Instance->CFGR);
#endif /* DEFINED ADC_MULTIMODE_SUPPORT */
...
HAL_StatusTypeDef HAL_ADC_ConfigChannel(ADC_HandleTypeDef *hadc, ADC_ChannelConfTypeDef *pConfig)
{
  ...
  if (hadc->Init.OversamplingMode == ENABLE)
  {
    assert_param(IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc), pConfig->Offset / (hadc->Init.Oversampling.Ratio + 1U)));
  }
  else
  {
  ...
  if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL)
  {
       ...
       )
    {
      ...
      if (pConfig->OffsetNumber != ADC_OFFSET_NONE)
      {
        ...
        LL_ADC_SetOffsetSaturation(hadc->Instance, pConfig->OffsetNumber, (pConfig->OffsetSaturation == ENABLE)
      ...
      if (pConfig->SingleDiff == ADC_DIFFERENTIAL_ENDED)
      {
        ...
        LL_ADC_SetChannelSamplingTime(hadc->Instance,
                                      (uint32_t)(__LL_ADC_DECIMAL_NB_TO_CHANNEL(
                                                   (__LL_ADC_CHANNEL_TO_DECIMAL_NB((uint32_t)pConfig->Channel) + 1UL)
                                                   & 0x1FUL)),
                   ...
                   ((tmp_config_internal_channel & LL_ADC_PATH_INTERNAL_VBAT) == 0UL))
          {
            if (ADC_BATTERY_VOLTAGE_INSTANCE(hadc))
            {
              LL_ADC_SetCommonPathInternalCh(__LL_ADC_COMMON_INSTANCE(hadc->Instance),
                   ...
                   ((pConfig->Channel == ADC_CHANNEL_VREFINT_ADC1) &&
                    ((tmp_config_internal_channel & LL_ADC_PATH_INTERNAL_VREFINT) == 0UL)))
          {
            if (ADC_VREFINT_INSTANCE(hadc))
            {
              LL_ADC_SetCommonPathInternalCh(__LL_ADC_COMMON_INSTANCE(hadc->Instance),
                   ...
                   && ((tmp_config_internal_channel & LL_ADC_PATH_INTERNAL_VDDCORE) == 0UL))
          {
            if (ADC_VDDCORE_INSTANCE(hadc))
            {
              LL_ADC_SetCommonPathInternalCh(__LL_ADC_COMMON_INSTANCE(hadc->Instance),
                                             LL_ADC_PATH_INTERNAL_VDDCORE | tmp_config_internal_channel);
            }
          }
          else if ((pConfig->Channel == ADC_CHANNEL_VDDCPU)
                   && ((tmp_config_internal_channel & LL_ADC_PATH_INTERNAL_VDDCPU) == 0UL))
          {
            if (ADC_VDDCPU_INSTANCE(hadc))
            {
              LL_ADC_SetCommonPathInternalCh(__LL_ADC_COMMON_INSTANCE(hadc->Instance),
                                             LL_ADC_PATH_INTERNAL_VDDCPU | tmp_config_internal_channel);
            }
          }
          else if ((pConfig->Channel == ADC_CHANNEL_VDDQ_DDR)
                   && ((tmp_config_internal_channel & LL_ADC_PATH_INTERNAL_VDDQ_DDR) == 0UL))
          {
            if (ADC_VDDQ_DDR_INSTANCE(hadc))
            {
              LL_ADC_SetCommonPathInternalCh(__LL_ADC_COMMON_INSTANCE(hadc->Instance),
                                             LL_ADC_PATH_INTERNAL_VDDQ_DDR | tmp_config_internal_channel);
            }
          }
          else
          {
...
HAL_StatusTypeDef HAL_ADC_AnalogWDGConfig(ADC_HandleTypeDef *hadc, ADC_AnalogWDGConfTypeDef *pAnalogWDGConfig)
{
     ...
     )
  {
    ...
    if (pAnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_1)
    {
      ...
      switch (pAnalogWDGConfig->WatchdogMode)
      {
        case ADC_ANALOGWATCHDOG_SINGLE_REG:
          LL_ADC_SetAnalogWDMonitChannels(hadc->Instance,
                                          LL_ADC_AWD1, __LL_ADC_ANALOGWD_CHANNEL_GROUP(pAnalogWDGConfig->Channel,
      ...
      MODIFY_REG(hadc->Instance->TR1,  ADC_TR1_LT1, tmp_awd_low_threshold_shifted);
      MODIFY_REG(hadc->Instance->TR1,  ADC_TR1_HT1, (tmp_awd_high_threshold_shifted << ADC_TR1_HT1_Pos));
    ...
    else
    {
      ...
      if (pAnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_2)
      {
        ...
        MODIFY_REG(hadc->Instance->TR2,  ADC_TR2_LT2, tmp_awd_low_threshold_shifted);
        MODIFY_REG(hadc->Instance->TR2,  ADC_TR2_HT2, (tmp_awd_high_threshold_shifted << ADC_TR2_HT2_Pos));
      }
      else
      {
        /* Set ADC analog watchdog thresholds value of both thresholds high and low */
        MODIFY_REG(hadc->Instance->TR3,  ADC_TR3_LT3, tmp_awd_low_threshold_shifted);
        MODIFY_REG(hadc->Instance->TR3,  ADC_TR3_HT3, (tmp_awd_high_threshold_shifted << ADC_TR3_HT3_Pos));
      }
