/**
  ...
  *** Callback registration ***
  ...
  Function @ref HAL_RTC_RegisterCallback() allows to register following callbacks:
    ...
    (+) InternalTamper10EventCallback : RTC InternalTamper 10 Event callback.
...
HAL_StatusTypeDef HAL_RTC_DeInit(RTC_HandleTypeDef *hrtc)
{
  ...
  if (status == HAL_OK)
  {
    ...
    WRITE_REG(RTC->SCR, RTC_SCR_CITSF | RTC_SCR_CTSOVF | RTC_SCR_CTSF | \
              RTC_SCR_CWUTF | RTC_SCR_CALRBF | RTC_SCR_CALRAF);
#if defined (CORTEX_IN_SECURE_STATE)
    WRITE_REG(RTC->SECCFGR, (RTC_SECCFGR_SEC | RTC_SECCFGR_INITSEC | RTC_SECCFGR_CALSEC | \
                             RTC_SECCFGR_TSSEC | RTC_SECCFGR_WUTSEC | RTC_SECCFGR_ALRBSEC | RTC_SECCFGR_ALRASEC));
...
/**
  ...
  *          @arg @ref HAL_RTC_INTERNAL_TAMPER10_EVENT_CB_ID Internal Tamper 10 Callback ID
...
/**
  ...
  * @param  hrtc RTC handle
  * @param  sTime Pointer to Time structure if Binary mode is RTC_BINARY_ONLY, this parameter
  * is not used and RTC_SSR will be automatically reset to 0xFFFFFFFF else sTime->SubSeconds is
  * not used and RTC_SSR will be automatically reset to the A 7-bit async prescaler (RTC_PRER_PREDIV_A)
...
HAL_StatusTypeDef HAL_RTC_SetAlarm(RTC_HandleTypeDef *hrtc, RTC_AlarmTypeDef *sAlarm, uint32_t Format)
{
  ...
  else /* RTC_BINARY_MIX */
  {
    ...
    assert_param(IS_RTC_ALARM_MASK(sAlarm->AlarmMask));
    assert_param(IS_RTC_ALARM_DATE_WEEKDAY_SEL(sAlarm->AlarmDateWeekDaySel));
    /* In Binary Mix Mode, the RTC can not generate an alarm on a
    match involving all calendar items + the upper SSR bits */
    assert_param((sAlarm->AlarmSubSecondMask >> RTC_ALRMASSR_MASKSS_Pos) <= (8U + \
                                                                             (READ_BIT(RTC->ICSR, RTC_ICSR_BCDU)\
                                                                              >> RTC_ICSR_BCDU_Pos)));
  }
