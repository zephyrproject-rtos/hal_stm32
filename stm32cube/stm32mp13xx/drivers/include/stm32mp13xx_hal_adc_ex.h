...
#ifndef STM32MP13xx_HAL_ADC_EX_H
#define STM32MP13xx_HAL_ADC_EX_H
...
/**
  ...
  *            modify parameter 'InjectedSingleDiff')
  *          - For parameters 'InjectedDiscontinuousConvMode', 'QueueInjectedContext', 'InjecOversampling': ADC
  *            enabled without conversion on going on injected group.
  *          - For parameters 'InjectedSamplingTime', 'InjectedOffset', 'InjectedOffsetNumber', 'InjectedOffsetSign',
  *            'InjectedOffsetSaturation', 'AutoInjectedConv': ADC enabled without conversion on going on regular
  *            and injected groups.
  *          - For parameters 'InjectedChannel', 'InjectedRank', 'InjectedNbrOfConversion',
  *             'ExternalTrigInjecConv', 'ExternalTrigInjecConvEdge': ADC enabled and while
  *             conversion on going on ADC groups regular and injected.
  *         If ADC is not in the appropriate state to modify some parameters, these parameters setting is
  *         bypassed without error reporting (as it can be the expected behavior in case of intended action
  *         to update another parameter (which fulfills the ADC state condition) on the fly).
  */
typedef struct
{
  ...
  uint32_t InjectedSamplingTime;          /*!< Sampling time value to be set for the selected channel.
                                               ...
                                               (12.5 ADC clock cycles at ADC resolution 12 bits, 10.5 cycles
                                               at 10 bits, 8.5 cycles at 8 bits, 6.5 cycles at 6 bits).
                                               ...
                                               in a regular and/or injected group.
                                                        It overwrites the last setting.
                                               Note: In case of usage of internal measurement channels
                                               (VrefInt/Vbat/TempSensor),
                                                     sampling time constraints must be respected (sampling
                                                     time can be adjusted in function of ADC clock frequency
  ...
  uint32_t InjectedSingleDiff;            /*!< Selection of single-ended or differential input.
                                               ...
                                               Caution: This parameter applies to a channel that can be
                                               used in a regular and/or injected group.
                                                        It overwrites the last setting.

                                               Note: Refer to Reference Manual to ensure the selected channel
                                               is available in differential mode.

                                               Note: When configuring a channel 'i' in differential mode, the
                                               channel 'i+1' is not usable separately.

                                               Note: This parameter must be modified when ADC is disabled
                                               (before ADC start conversion or after ADC stop conversion).

                                               If ADC is enabled, this parameter setting is bypassed without
                                               error reporting (as it can be the expected behavior in case
  ...
  uint32_t InjectedOffset;                /*!< Defines the offset to be subtracted from the raw converted data.
                                               ...
                                               between Min_Data = 0x000 and Max_Data = 0xFFF,  0x3FF, 0xFF or
                                               0x3F respectively.
                                               Note: This parameter must be modified when no conversion is on
                                               going on both regular and injected groups (ADC disabled, or ADC
                                               enabled without continuous mode or external trigger that could
                                               launch a conversion). */

  /* ADC_FEATURE_OFFSET_COMPENSATION */
  uint32_t InjectedOffsetSign;                /*!< Define if the offset should be subtracted (negative sign)
  ...
  FunctionalState InjectedOffsetSaturation;   /*!< Define if the offset should be saturated upon under or
                                                   over flow.
  ...
  uint32_t InjectedNbrOfConversion;       /*!< Specifies the number of ranks that will be converted within the
  ...
  FunctionalState InjectedDiscontinuousConvMode; /*!< Specifies whether the conversions sequence of ADC group
                                               injected is performed in Complete-sequence/Discontinuous-sequence
                                               ...
                                               Note: This parameter must be modified when ADC is disabled (before
  ...
  FunctionalState AutoInjectedConv;       /*!< Enables or disables the selected ADC group injected automatic conversion after
                                               regular one This parameter can be set to ENABLE or DISABLE.
                                               ...
                                               Note: In case of DMA used with regular group: if DMA configured in
                                                     normal mode (single shot) JAUTO will be stopped upon DMA
                                                     transfer complete.
  ...
  uint32_t ExternalTrigInjecConv;         /*!< Selects the external event used to trigger the conversion start of injected group.
                                               If set to ADC_INJECTED_SOFTWARE_START, external triggers are disabled and
                                               software trigger is used instead.
                                               This parameter can be a value of
                                               @ref ADC_injected_external_trigger_source.
                                               Caution: this setting impacts the entire injected group. Therefore, call
                                                        of HAL_ADCEx_InjectedConfigChannel() to configure a channel on
  ...
  ADC_InjOversamplingTypeDef  InjecOversampling; /*!< Specifies the Oversampling parameters.
                                                      ...
                                                      Note: This parameter can be modified only if there
                                                      is no conversion is ongoing (both ADSTART and JADSTART
                                                      cleared). */
...
/**
  ...
  * @note   The setting of these parameters by function HAL_ADCEx_MultiModeConfigChannel() is conditioned by
  *         ADCs state (both Master and Slave ADCs).
...
#define ADC_TWOSAMPLINGDELAY_8CYCLES       (LL_ADC_MULTI_TWOSMP_DELAY_8CYCLES_5)  /*!< ADC multimode delay between two sampling phases: 8 ADC clock cycles */
/**
  * @}
  */
...
/**
  ...
  * @retval None
  */
#define ADC_CFGR_INJECT_CONTEXT_QUEUE(__INJECT_CONTEXT_QUEUE_MODE__) ((__INJECT_CONTEXT_QUEUE_MODE__)\
                                                                      << ADC_CFGR_JQM_Pos)
...
/**
  ...
  * @retval None
  */
#define ADC_CFGR_INJECT_DISCCONTINUOUS(__INJECT_DISCONTINUOUS_MODE__) ((__INJECT_DISCONTINUOUS_MODE__)\
...
/**
  ...
  * @retval None
  */
#define ADC_CFGR_DISCONTINUOUS_NUM(__NBR_DISCONTINUOUS_CONV__) (((__NBR_DISCONTINUOUS_CONV__) - 1UL)\
...
/**
  ...
  */
#define ADC1_COMMON_REGISTER(__HANDLE__)   (ADC1_COMMON)
#define ADC2_COMMON_REGISTER(__HANDLE__)   (ADC2_COMMON)
...
#if defined(ADC_MULTIMODE_SUPPORT)
#define ADC_CLEAR_COMMON_CONTROL_REGISTER(__HANDLE__) CLEAR_BIT(__LL_ADC_COMMON_INSTANCE(\
                                                                (__HANDLE__)->Instance)->CCR, ADC_CCR_CKMODE | \
                                                                ADC_CCR_PRESC     | \
                                                                ADC_CCR_VBATEN    | \
                                                                ADC_CCR_VSENSEEN  | \
                                                                ADC_CCR_VREFEN    | \
...
/**
  * @brief Verify the ADC instance connected to the internal voltage reference VDDCPU.
  * @param __HANDLE__ ADC handle.
  * @retval SET (ADC instance is valid) or RESET (ADC instance is invalid)
  */
...
/**
  * @brief Verify the ADC instance connected to the internal voltage reference VDDQ_DDR.
  * @param __HANDLE__ ADC handle.
  * @retval SET (ADC instance is valid) or RESET (ADC instance is invalid)
  */
#define ADC_VDDQ_DDR_INSTANCE(__HANDLE__)  (((__HANDLE__)->Instance) == ADC2)
...
#define IS_ADC_CHANNEL(__CHANNEL__)                 (((__CHANNEL__) == ADC_CHANNEL_0)           || \
                                                     ...
                                                     ((__CHANNEL__) == ADC_CHANNEL_VREFINT_ADC1)|| \
                                                     ((__CHANNEL__) == ADC_CHANNEL_VCORE)       || \
                                                     ((__CHANNEL__) == ADC_CHANNEL_VBAT)        || \
                                                     ((__CHANNEL__) == ADC_CHANNEL_VDDCPU)      || \
                                                     ((__CHANNEL__) == ADC_CHANNEL_VDDQ_DDR)       )
...
/**
  ...
  */
#define IS_ADC_ANALOG_WATCHDOG_MODE(\
                                    __WATCHDOG_MODE__)\
...
#endif /* STM32MP13xx_HAL_ADC_EX_H */
