...
#ifndef STM32MP13xx_HAL_ADC_H
#define STM32MP13xx_HAL_ADC_H
...
typedef struct
{
  ...
  uint32_t OversamplingStopReset;         /*!< Selects the regular oversampling mode.
                                               ...
                                               during injection sequence). This parameter can be a value of @ref
...
typedef struct
{
  uint32_t ClockPrescaler;        /*!< Select ADC clock source (synchronous clock derived from APB clock or asynchronous clock derived from system clock or PLL (Refer to reference manual for list of clocks available)) and clock prescaler.
                                       ...
                                       Note: In case of usage of channels on injected group, ADC frequency should be
                                             ...
                                             frequency /3 for resolution 8 bits, AHB clock frequency /2 for resolution
                                             6 bits.
  ...
  uint32_t ScanConvMode;          /*!< Configure the sequencer of ADC groups regular and injected.
                                       ...
                                       If disabled: Conversion is performed in single mode (one channel converted,
                                                    the one defined in rank 1). Parameters 'NbrOfConversion' and
                                       ...
                                       If enabled:  Conversions are performed in sequence mode (multiple ranks
                                                    defined by 'NbrOfConversion' or 'InjectedNbrOfConversion' and
                                                    rank of each channel in sequencer).
                                                    Scan direction is upward: from rank 1 to rank 'n'.
                                       This parameter can be a value of @ref ADC_Scan_mode */

  uint32_t EOCSelection;          /*!< Specify which EOC (End Of Conversion) flag is used for conversion by polling
  ...
  FunctionalState LowPowerAutoWait; /*!< Select the dynamic low power Auto Delay: new conversion start only when the
                                       ...
                                       This feature automatically adapts the frequency of ADC conversions triggers
                                       to the speed of the system that reads the data. Moreover, this avoids risk
                                       ...
                                       Note: It is not recommended to use with interruption or DMA (HAL_ADC_Start_IT(),
                                             ...
                                             on, when ADC conversion data is needed: use HAL_ADC_PollForConversion()
                                              to ensure that conversion is completed and HAL_ADC_GetValue() to retrieve
                                               conversion result and trig another conversion start.
  ...
  uint32_t NbrOfConversion;       /*!< Specify the number of ranks that will be converted within the regular group sequencer.
                                       To use the regular group sequencer and convert several ranks, parameter
                                       'ScanConvMode' must be enabled.
                                       This parameter must be a number between Min_Data = 1 and Max_Data = 16.
                                       Note: This parameter must be modified when no conversion is on going on regular
                                       group (ADC disabled, or ADC enabled without
                                       continuous mode or external trigger that could launch a conversion). */

  FunctionalState DiscontinuousConvMode; /*!< Specify whether the conversions sequence of ADC group regular is performed in
                                       Complete-sequence/Discontinuous-sequence (main sequence subdivided
                                       in successive parts).
  ...
  uint32_t ExternalTrigConv;      /*!< Select the external event source used to trigger ADC group regular conversion start.
                                       ...
                                       @ref ADC_regular_external_trigger_source.
  ...
  uint32_t Overrun;               /*!< Select the behavior in case of overrun: data overwritten or preserved (default).
                                       ...
                                       model with interruption (HAL_Start_IT()): ADC IRQ handler has to clear
                                       end of conversion flags, this induces the release of the preserved data.
                                       If needed, this data can be saved in function
                                       HAL_ADC_ConvCpltCallback(), placed in user program code (called before
                                       end of conversion flags clear).
                                       Note: Error reporting with respect to the conversion mode:
                                             - Usage with ADC conversion by polling for event or interruption:
                                               Error is reported only if overrun is set to data preserved. If
                                               overrun is set to data
                                             ...
                                             - Usage with ADC conversion by DMA: Error is reported whatever
                                               overrun setting (DMA is expected to process all data from
                                               data register). */
...
/**
  ...
  *         If ADC is not in the appropriate state to modify some parameters, these parameters setting is bypassed
  *         without error reporting (as it can be the expected behavior in case of intended action to update another
  *         parameter (which fulfills the ADC state condition)
  *         on the fly).
  */
typedef struct
{
  ...
  uint32_t SamplingTime;           /*!< Sampling time value to be set for the selected channel.
                                        ...
                                        Note: In case of usage of internal measurement channels
                                              ...
                                              respected (sampling time can be adjusted
  ...
  uint32_t SingleDiff;             /*!< Select single-ended or differential input.
                                        ...
                                        the selected channel 'i' (positive input) and channel 'i+1' (negative input).
                                        ...
                                        Note: This parameter must be modified when ADC is disabled (before ADC start
                                              ...
                                              reporting (as it can be the expected behavior in case
...
/**
  ...
  *         ADC state can be either:
  *          - For all parameters: ADC disabled or ADC enabled without conversion on going on ADC groups
...
typedef struct
{
  uint32_t WatchdogNumber;    /*!< Select which ADC analog watchdog is monitoring the selected channel.
                                   ...
                                   For Analog Watchdog 2 and 3: Several channels can be monitored (by successive
  ...
  uint32_t WatchdogMode;      /*!< Configure the ADC analog watchdog mode: single/all/none channels.
                                   ...
                                   successively the AWD init structure. Channels on ADC group regular and injected
                                   are not differentiated: Set value 'ADC_ANALOGWATCHDOG_SINGLE_xxx' to monitor 1
                                   channel, value 'ADC_ANALOGWATCHDOG_ALL_xxx' to monitor all channels,
  ...
  uint32_t Channel;           /*!< Select which ADC channel to monitor by analog watchdog.
                                   ...
                                   For Analog Watchdog 2 and 3: Several channels can be monitored. To use this feature,
                                    call successively the function HAL_ADC_AnalogWDGConfig() for each channel to be
                                    added (or removed with value 'ADC_ANALOGWATCHDOG_NONE').
                                   This parameter can be a value of @ref ADC_HAL_EC_CHANNEL. */
  ...
  uint32_t HighThreshold;     /*!< Configure the ADC analog watchdog High threshold value.
                                   Depending of ADC resolution selected (16, 14, 12, 10, 8 bits), this parameter
                                   must be a number between Min_Data = 0x000 and Max_Data = 0xFFFF, 0x3FFF,
                                   0xFFF, 0x3FF or 0xFF respectively.
                                   Note: Analog watchdog 2 and 3 are limited to a resolution of 8 bits: if ADC
                                         resolution is 12 bits the 4 LSB are ignored, if ADC resolution is 10 bits
                                   ...
                                   Note: If ADC oversampling is enabled, ADC analog watchdog thresholds are
                                         ...
                                         application): intermediate register bitfield [32:7] (26 most
                                         significant bits). */
  ...
  uint32_t LowThreshold;      /*!< Configures the ADC analog watchdog Low threshold value.
                                   ...
                                   Note: Analog watchdog 2 and 3 are limited to a resolution of 8 bits:
                                         if ADC resolution is 12 bits the 4 LSB are ignored, if ADC resolution
  ...
  uint32_t FilteringConfig;   /*!< Specify whether filtering should be use and the number of samples to consider.
                                   ...
                                   This parameter only applies to Analog watchdog 1. For others,
                                   use value ADC_AWD_FILTERING_NONE.
...
#define HAL_ADC_STATE_INJ_BUSY          (0x00001000UL)   /*!< A conversion on ADC group injected is ongoing or can occur (either by auto-injection mode, external trigger, low power auto power-on (if feature available), multimode ADC master control (if feature available)) */
...
#define ADC_DATAALIGN_LEFT                 (LL_ADC_DATA_ALIGN_LEFT)       /*!< ADC conversion data alignment: left aligned (aligmnent on data register MSB bit 15)*/
/**
  * @}
  */
...
#define ADC_SAMPLING_MODE_BULB                  (ADC_CFGR2_BULB)    /*!< ADC conversions sampling phase starts immediately after end of conversion, and stops upon trigger event. Notes: - First conversion is using minimal sampling time (see @ref ADC_HAL_EC_CHANNEL_SAMPLINGTIME) */
#define ADC_SAMPLING_MODE_TRIGGER_CONTROLED     (ADC_CFGR2_SMPTRIG) /*!< ADC conversions sampling phase is controlled by trigger events: Trigger rising edge  = start sampling, Trigger falling edge = stop sampling and start conversion */
/**
  * @}
  */
...
#define ADC_CHANNEL_VREFINT                (LL_ADC_CHANNEL_VREFINT)         /*!< ADC internal channel connected to VrefInt: Internal voltage reference, channel specific to ADC2. */
#define ADC_CHANNEL_VCORE                  (LL_ADC_CHANNEL_VCORE)           /*!< ADC internal channel connected to VddCore, channel specific to ADC2. */
#define ADC_CHANNEL_TEMPSENSOR             (LL_ADC_CHANNEL_TEMPSENSOR)      /*!< ADC internal channel connected to Temperature sensor, channel specific to ADC2. */
#define ADC_CHANNEL_VBAT                   (LL_ADC_CHANNEL_VBAT)            /*!< ADC internal channel connected to Vbat/4: Vbat voltage through a divider ladder of factor 1/4 to have Vbat always below Vdda, channel specific to ADC2. */
#define ADC_CHANNEL_VDDCPU                 (LL_ADC_CHANNEL_VDDCPU)          /*!< ADC internal channel connected to VDDCPU, channel specific to ADC2 */
#define ADC_CHANNEL_VDDQ_DDR               (LL_ADC_CHANNEL_VDDQ_DDR)        /*!< ADC internal channel connected to VDDQ_DDR, channel specific to ADC2 */
#define ADC_CHANNEL_VREFINT_ADC1           (LL_ADC_CHANNEL_VREFINT_ADC1)    /*!< ADC internal channel connected to VrefInt: Internal voltage reference, channel specific to ADC1. */
/**
  * @}
  */
...
#define ADC_AWD_FILTERING_2SAMPLES      ((ADC_TR1_AWDFILT_0))                                            /*!< ADC analog watchdog 2 consecutives out-of-window samples are needed to raise flag or interrupt. */
#define ADC_AWD_FILTERING_3SAMPLES      ((ADC_TR1_AWDFILT_1))                                            /*!< ADC analog watchdog 3 consecutives out-of-window samples are needed to raise flag or interrupt. */
#define ADC_AWD_FILTERING_4SAMPLES      ((ADC_TR1_AWDFILT_1 | ADC_TR1_AWDFILT_0))                        /*!< ADC analog watchdog 4 consecutives out-of-window samples are needed to raise flag or interrupt. */
#define ADC_AWD_FILTERING_5SAMPLES      ((ADC_TR1_AWDFILT_2))                                            /*!< ADC analog watchdog 5 consecutives out-of-window samples are needed to raise flag or interrupt. */
#define ADC_AWD_FILTERING_6SAMPLES      ((ADC_TR1_AWDFILT_2 | ADC_TR1_AWDFILT_0))                        /*!< ADC analog watchdog 6 consecutives out-of-window samples are needed to raise flag or interrupt. */
#define ADC_AWD_FILTERING_7SAMPLES      ((ADC_TR1_AWDFILT_2 | ADC_TR1_AWDFILT_1))                        /*!< ADC analog watchdog 7 consecutives out-of-window samples are needed to raise flag or interrupt. */
#define ADC_AWD_FILTERING_8SAMPLES      ((ADC_TR1_AWDFILT_2 | ADC_TR1_AWDFILT_1 | ADC_TR1_AWDFILT_0))    /*!< ADC analog watchdog 8 consecutives out-of-window samples are needed to raise flag or interrupt. */
/**
  * @}
  */
...
#define ADC_AWD1_EVENT           (ADC_FLAG_AWD1)  /*!< ADC Analog watchdog 1 event (main analog watchdog, present on all STM32 seriess) */
#define ADC_AWD2_EVENT           (ADC_FLAG_AWD2)  /*!< ADC Analog watchdog 2 event (additional analog watchdog, not present on all STM32 seriess) */
#define ADC_AWD3_EVENT           (ADC_FLAG_AWD3)  /*!< ADC Analog watchdog 3 event (additional analog watchdog, not present on all STM32 seriess) */
...
#define ADC_IS_SYNCHRONOUS_CLOCK_MODE(__HANDLE__)                                   \
  (((((__HANDLE__)->Instance) == ADC1) || (((__HANDLE__)->Instance) == ADC2))? \
   ((ADC12_COMMON->CCR & ADC_CCR_CKMODE) != 0UL): RESET)
...
/**
  ...
  * @retval SET (__LENGTH__ is within the maximum number of possible programmable regular conversions) or
...
/**
  ...
  *         (3) On STM32MP13, fast channel (0.125 us for 14-bit resolution (ADC conversion rate up to 8 Ms/s)).
...
/**
  ...
  *                                        uV/DegCelsius).On STM32MP13, refer to device datasheet parameter "Avg_Slope".
  * @param  __TEMPSENSOR_TYP_CALX_V__     Device datasheet data: Temperature sensor voltage typical value (at
  *                                       temperature and Vref+ defined in parameters below) (unit: mV). On STM32MP13,
...
#include "stm32mp13xx_hal_adc_ex.h"
