...
#ifndef STM32MP13xx_HAL_MDMA_H
#define STM32MP13xx_HAL_MDMA_H
...
typedef struct
{
  ...
  uint32_t TransferTriggerMode;     /*!< Specifies the Trigger Transfer mode : each request triggers a :
                                         a buffer transfer, a block transfer, a repeated block transfer
                                         or a linked list transfer
  ...
  uint32_t BufferTransferLength;      /*!< Specifies the buffer Transfer Length (number of bytes),
                                           this is the number of bytes to be transferred in a single transfer
                                           (1 byte to 128 bytes)*/
  ...
  uint32_t SourceBurst;              /*!< Specifies the Burst transfer configuration for the source memory transfers.
                                          ...
                                          the BURST must be programmed as to ensure that the burst size will be lower
                                          than BufferTransferLength */

  uint32_t DestBurst;                 /*!< Specifies the Burst transfer configuration for the destination memory
                                           transfers.
                                           It specifies the amount of data to be transferred in a single non
                                           interruptible transaction.
  ...
  int32_t SourceBlockAddressOffset;   /*!< this field specifies the Next block source address offset
                                           signed value : if > 0 then  increment the next block source Address by
                                                          offset from where the last block ends
                                                          if < 0 then  decrement the next block source Address by
                                                          offset from where the last block ends
                                                          if == 0, the next block source address starts from where
                                                          the last block ends
                                       */


  int32_t DestBlockAddressOffset;      /*!< this field specifies the Next block destination address offset
                                            signed value : if > 0 then  increment the next block destination Address by
                                                           offset from where the last block ends
                                                           if < 0 then  decrement the next block destination Address by
                                                           offset from where the last block ends
                                                           if == 0, the next block destination address starts from where
                                                           the last block ends
                                       */
...
typedef struct
{
  ...
  uint32_t PostRequestMaskAddress;  /*!< specifies the address to be updated (written) with PostRequestMaskData after
                                         a request is served.
                                         PostRequestMaskAddress and PostRequestMaskData could be used to automatically
                                         clear a peripheral flag when the request is served  */
...
typedef struct __MDMA_HandleTypeDef
{
  ...
  MDMA_LinkNodeTypeDef *FirstLinkedListNodeAddress;                                             /*!< specifies the
                                                                                                     first node
                                                                                                     address of the
                                                                                                     transfer list
                                                                                                     (after the initial
...
#if defined (DMA3)
#define MDMA_REQUEST_DMA3_Stream0_TC      ((uint32_t)0x00000010U)  /*!< MDMA HW request is DMA3 Stream 0 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA3_Stream1_TC      ((uint32_t)0x00000011U)  /*!< MDMA HW request is DMA3 Stream 1 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA3_Stream2_TC      ((uint32_t)0x00000012U)  /*!< MDMA HW request is DMA3 Stream 2 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA3_Stream3_TC      ((uint32_t)0x00000013U)  /*!< MDMA HW request is DMA3 Stream 3 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA3_Stream4_TC      ((uint32_t)0x00000014U)  /*!< MDMA HW request is DMA3 Stream 4 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA3_Stream5_TC      ((uint32_t)0x00000015U)  /*!< MDMA HW request is DMA3 Stream 5 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA3_Stream6_TC      ((uint32_t)0x00000016U)  /*!< MDMA HW request is DMA3 Stream 6 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA3_Stream7_TC      ((uint32_t)0x00000017U)  /*!< MDMA HW request is DMA3 Stream 7 Transfer Complete Flag   */
#endif /* DMA3 */
#define MDMA_REQUEST_FMC_DATA             ((uint32_t)0x00000018U)  /*!< MDMA HW request is NAND data transfer (Tx or Rx) channel  */
#define MDMA_REQUEST_FMC_ERROR            ((uint32_t)0x00000019U)  /*!< MDMA HW request is NAND ECC/BCH Error channel             */
#define MDMA_REQUEST_QUADSPI_FIFO_TH      ((uint32_t)0x0000001AU)  /*!< MDMA HW request is QSPI FIFO threshold Flag               */
#define MDMA_REQUEST_QUADSPI_TC           ((uint32_t)0x0000001BU)  /*!< MDMA HW request is QSPI Transfer complete Flag            */
#define MDMA_REQUEST_CRYP1_IN             ((uint32_t)0x0000001CU)  /*!< MDMA HW request is CRYP1 4 word request from input        */
#define MDMA_REQUEST_CRYP1_OUT            ((uint32_t)0x0000001DU)  /*!< MDMA HW request is CRYP1 4 word request from output       */
#define MDMA_REQUEST_HASH1_IN             ((uint32_t)0x0000001EU)  /*!< MDMA HW request is HASH1 16 word request from input       */
#define MDMA_REQUEST_SAES_IN              ((uint32_t)0x0000001FU)  /*!< MDMA HW request is SAES request from input        */
#define MDMA_REQUEST_SAES_OUT             ((uint32_t)0x00000020U)  /*!< MDMA HW request is SAES request from output       */
...
/**
  ...
  * @retval None
  */
#define __HAL_MDMA_SET_COUNTER(__HANDLE__, __COUNTER__)  ((__HANDLE__)->Instance->CBNDTR |= ((__COUNTER__) &\
                                                          MDMA_CBNDTR_BNDT))
...
HAL_StatusTypeDef HAL_MDMA_RegisterCallback(MDMA_HandleTypeDef *hmdma, HAL_MDMA_CallbackIDTypeDef CallbackID,
...
HAL_StatusTypeDef HAL_MDMA_LinkedList_AddNode(MDMA_HandleTypeDef *hmdma, MDMA_LinkNodeTypeDef *pNewNode,
                                              const MDMA_LinkNodeTypeDef *pPrevNode);
...
/** @defgroup MDMA_Exported_Functions_Group3 I/O operation functions
  ...
  * @{
  */
HAL_StatusTypeDef HAL_MDMA_Start(MDMA_HandleTypeDef *hmdma, uint32_t SrcAddress, uint32_t DstAddress,
                                 uint32_t BlockDataLength, uint32_t BlockCount);
HAL_StatusTypeDef HAL_MDMA_Start_IT(MDMA_HandleTypeDef *hmdma, uint32_t SrcAddress, uint32_t DstAddress,
                                    uint32_t BlockDataLength, uint32_t BlockCount);
HAL_StatusTypeDef HAL_MDMA_Abort(MDMA_HandleTypeDef *hmdma);
HAL_StatusTypeDef HAL_MDMA_Abort_IT(MDMA_HandleTypeDef *hmdma);
HAL_StatusTypeDef HAL_MDMA_PollForTransfer(MDMA_HandleTypeDef *hmdma, HAL_MDMA_LevelCompleteTypeDef CompleteLevel,
                                           uint32_t Timeout);
...
#define IS_MDMA_REQUEST(__REQUEST__) (((__REQUEST__) == MDMA_REQUEST_SW ) || ((__REQUEST__) <= MDMA_REQUEST_SAES_OUT))
...
#define IS_MDMA_BLOCK_ADDR_OFFSET(__BLOCK_ADD_OFFSET__) (((__BLOCK_ADD_OFFSET__) > (-65536)) &&\
                                                         ((__BLOCK_ADD_OFFSET__) < 65536))
