...
#ifndef STM32MP13xx_HAL_LTDC_H
#define STM32MP13xx_HAL_LTDC_H
...
typedef struct
{
  ...
  uint32_t FrameBufferPich;            /*!< Frame buffer pitch in bytes */
...
typedef struct
{
  ...
  uint8_t BlendingOrder;               /*!< Select the blending order.
                                        This parameter can be one of between Min_Data = 0x00 and Max_Data = 0x0F. */

  uint8_t BurstLength;                   /*!< Select the burst length.
                                        This parameter must be a number between Min_Data = 0x00 and Max_Data = 0x10. */
  LTDC_PlanarTypeDef   Planar;     /*!< Configures the planar or semiplanar buffer. */

  bool HorMirrorEn;                /*!< Horizontal mirroring Enable. */

  bool VertMirrorEn;               /*!< Vertical mirroring Enable. */
...
/** @defgroup LTDC_Private_Macros LTDC Private Macros
  * @{
  */
#define LTDC_LAYER(__HANDLE__, __LAYER__)       ((LTDC_Layer_TypeDef *)((uint32_t)(((uint32_t)((__HANDLE__)->Instance))\
