...
HAL_StatusTypeDef HAL_LTDCEx_StructInitFromVideoConfig(LTDC_HandleTypeDef *hltdc, const DSI_VidCfgTypeDef *VidCfg)
{
  ...
  hltdc->Init.DEPolarity = (VidCfg->DEPolarity == DSI_DATA_ENABLE_ACTIVE_HIGH) \
                           ? LTDC_DEPOLARITY_AL : LTDC_DEPOLARITY_AH;
                                                                ...
                                                                const DSI_CmdCfgTypeDef *CmdCfg)
{
  ...
  /* Note 1 : Code in line w/ Current LTDC specification */
  hltdc->Init.DEPolarity = (CmdCfg->DEPolarity == DSI_DATA_ENABLE_ACTIVE_HIGH) \
                           ? LTDC_DEPOLARITY_AL : LTDC_DEPOLARITY_AH;
...
/**
  * @brief  enable external trigger (single-frame mode enable)
  * @param  hltdc   pointer to a LTDC_HandleTypeDef structure that contains
  *                 the configuration information for the LTDC.
  * @retval HAL status
  */
...
{
  hltdc->Instance->GCR |= (uint32_t)(LTDC_GCR_SFEXEN);
  return HAL_OK;
}

/**
  * @brief  disable external trigger (single-frame mode enable)
  * @param  hltdc   pointer to a LTDC_HandleTypeDef structure that contains
  *                 the configuration information for the LTDC.
  * @retval HAL status
  */
...
{
  hltdc->Instance->GCR &= ~(LTDC_GCR_SFEXEN);
  return HAL_OK;
}

/**
  * @brief  enable software trigger (single-frame mode enable)
  * @param  hltdc   pointer to a LTDC_HandleTypeDef structure that contains
  *                 the configuration information for the LTDC.
  * @retval HAL status
  */
...
{
  hltdc->Instance->GCR |= (uint32_t)(LTDC_GCR_SFSWTR);
  return HAL_OK;
}
