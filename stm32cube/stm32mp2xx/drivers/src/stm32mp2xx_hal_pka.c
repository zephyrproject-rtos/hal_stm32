...
void HAL_PKA_Arithmetic_GetResult(PKA_HandleTypeDef *hpka, uint32_t *pRes)
{
  ...
  switch (mode)
  {
    ...
    case PKA_MODE_MONTGOMERY_MUL:
      size = (hpka->Instance->RAM[2] + 31U) / 32UL;
      break;
    case PKA_MODE_ARITHMETIC_ADD:
    case PKA_MODE_MODULAR_SUB:
      size = (hpka->Instance->RAM[PKA_ARITHMETIC_ALL_OPS_NB_BITS] + 31U) / 32UL;
    ...
    case PKA_MODE_ARITHMETIC_MUL:
      size = (hpka->Instance->RAM[PKA_ARITHMETIC_MUL_NB_BITS] + 31U) / 32UL * 2UL;
      break;
    default:
      break;
  }
...
HAL_StatusTypeDef HAL_PKA_Abort(PKA_HandleTypeDef *hpka)
{
      ...
      (mode == PKA_MODE_ECDSA_SIGNATURE))
  {
    ...
    /* Wait until the PKA process of clearing has been complete */
    while ((hpka->Instance->SR & PKA_SR_INITOK) != PKA_SR_INITOK)
    {
