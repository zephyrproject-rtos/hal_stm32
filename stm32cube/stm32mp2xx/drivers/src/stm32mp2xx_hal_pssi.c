...
HAL_StatusTypeDef HAL_PSSI_Init(PSSI_HandleTypeDef *hpssi)
{
  ...
  assert_param(IS_PSSI_DATAWIDTH(hpssi->Init.DataWidth));
  ...
  if (hpssi->State == HAL_PSSI_STATE_READY)
  {
    ...
    if (hpssi->Init.DataWidth == HAL_PSSI_8BITS)
    {
      while (hpssi->XferCount > 0U)
      {
        ...
        /* Write data to DR */
        *((__IO uint8_t *)(&hpssi->Instance->DR)) = *((uint8_t *)hpssi->pBuffPtr);
        /* Increment Buffer pointer */
        hpssi->pBuffPtr  += sizeof(uint8_t);
        hpssi->XferCount -= sizeof(uint8_t);
      }
    }
    else if (hpssi->Init.DataWidth == HAL_PSSI_16BITS)
    {
      while (hpssi->XferCount > 0U)
      {
        ...
        /* Write data to DR */
#if defined (__GNUC__)
        *pdr_16bits = *((uint16_t *)hpssi->pBuffPtr);
#else
        *((__IO uint16_t *)(&hpssi->Instance->DR)) = *((uint16_t *)hpssi->pBuffPtr);
#endif /* __GNUC__ */
        /* Increment Buffer pointer */
        hpssi->pBuffPtr  += sizeof(uint16_t);
        hpssi->XferCount -= sizeof(uint16_t);
      }
    }
    else if (hpssi->Init.DataWidth == HAL_PSSI_32BITS)
    {
      while (hpssi->XferCount > 0U)
      {
        ...
        /* Write data to DR */
        *((__IO uint32_t *)(&hpssi->Instance->DR)) = *((uint32_t *)hpssi->pBuffPtr);
        /* Increment Buffer pointer */
        hpssi->pBuffPtr  += sizeof(uint32_t);
        hpssi->XferCount -= sizeof(uint32_t);
      }
  ...
  if (hpssi->State == HAL_PSSI_STATE_READY)
  {
    ...
    if (hpssi->Init.DataWidth == HAL_PSSI_8BITS)
    {
      while (hpssi->XferCount > 0U)
      {
        ...
        /* Read data from DR */
        *((uint8_t *)hpssi->pBuffPtr) = *(__IO uint8_t *)(&hpssi->Instance->DR);
    ...
    else if (hpssi->Init.DataWidth == HAL_PSSI_32BITS)
    {
      while (hpssi->XferCount > 0U)
      {
        ...
        /* Read data from DR */
        *((uint32_t *)hpssi->pBuffPtr) = *(__IO uint32_t *)(&hpssi->Instance->DR);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_PSSI_Transmit_DMA(PSSI_HandleTypeDef *hpssi, uint8_t *pData, uint32_t Size)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_PSSI_Receive_DMA(PSSI_HandleTypeDef *hpssi, uint8_t *pData, uint32_t Size)
{
  HAL_StatusTypeDef dmaxferstatus;
  if (((hpssi->Init.DataWidth == HAL_PSSI_8BITS) && (hpssi->Init.BusWidth != HAL_PSSI_8LINES)) ||
      ((hpssi->Init.DataWidth == HAL_PSSI_8BITS) && ((Size % 8U) != 0U))   ||
