  ...
  ******************************************************************************
  ...
  [..]
    ...
    (#) In case of external interrupt mode selection, configure NVIC IRQ priority
        ...
        (+) HAL_GPIO_EXTI_Rising_Callback() user callbacks will be executed :
          (++) on rising edge detection.
        (+) HAL_GPIO_EXTI_Rising_Callback() user callbacks will be executed :
          (++) on falling edge detection.
...
/**
  ...
  * @param  GPIOx where x can be (A..C, H).
...
/**
  ...
  * @param  GPIOx where x can be (A..H) to select the GPIO peripheral for STM32WBA family
