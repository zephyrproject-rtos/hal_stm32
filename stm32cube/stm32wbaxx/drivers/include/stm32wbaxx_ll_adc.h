...
#ifndef STM32WBAxx_LL_ADC_H
#define STM32WBAxx_LL_ADC_H
...
/* Internal voltage reference VrefInt */
#define VREFINT_CAL_ADDR                   ((const uint16_t*) (0x0BF8D7A5UL)) /* Internal voltage reference, address of
                                           parameter VREFINT_CAL: VrefInt ADC raw data acquired at temperature 30 DegC
                                           (tolerance: +-5 DegC), Vref+ = 3.3 V (tolerance: +-10 mV). */
#else
/* Internal voltage reference VrefInt */
...
#elif defined(STM32WBA20xx) || defined(STM32WBA23xx) || defined(STM32WBA25xx)
/* Temperature sensor */
#define TEMPSENSOR_CAL1_ADDR               ((const uint16_t*) (0x0BF8D710UL)) /* Address of parameter TS_CAL1: On this series,
                                           temperature sensor ADC raw data acquired at temperature  30 DegC
                                           (tolerance: +-5 DegC), Vref+ = 3.0 V (tolerance: +-10 mV). */
#define TEMPSENSOR_CAL2_ADDR               ((const uint16_t*) (0x0BF8D742UL)) /* Address of parameter TS_CAL2: On this series,
                                           temperature sensor ADC raw data acquired at temperature 130 DegC
                                           (tolerance: +-5 DegC), Vref+ = 3.0 V (tolerance: +-10 mV). */
#else
/* Temperature sensor */
...
#define LL_ADC_FLAG_LDORDY                 ADC_ISR_LDORDY     /*!< ADC flag ADC internal voltage regulator ready */
...
#define LL_ADC_IT_LDORDY                   ADC_IER_LDORDYIE   /*!< ADC interruption internal voltage regulator ready */
...
#define LL_ADC_LP_AUTOWAIT_ENABLE          (ADC_CFGR1_WAIT)   /*!< ADC low power mode auto delay enabled: dynamic
...
#define LL_ADC_LP_AUTOPOWEROFF_ENABLE       (ADC_PWRR_AUTOFF) /*!< ADC low power mode auto power-off enabled: the ADC
                                           ...
                                           @ref LL_ADC_SetLPModeAutoPowerOff().
...
#define LL_ADC_LP_AUTONOMOUS_DPD_ENABLE       (ADC_PWRR_DPD)  /*!< ADC deep power down in autonomous mode enabled */
/**
  * @}
  */
...
#define LL_ADC_CHANNEL_VCORE               (LL_ADC_CHANNEL_12 | ADC_CHANNEL_ID_INTERNAL_CH)  /*!< ADC internal channel
                                           connected to Vcore: internal power supply of MCU digital logic */
/**
  * @}
  */
...
#define LL_ADC_REG_TRIG_EXT_TIM2_TRGO      (ADC_CFGR1_EXTSEL_1 | ADC_REG_TRIG_EXT_EDGE_DEFAULT)  /*!< ADC group regular
                                           ...
                                           Trigger edge set to rising edge (default setting). */
#define LL_ADC_REG_TRIG_EXT_LPTIM1_CH1     (ADC_CFGR1_EXTSEL_2 | ADC_CFGR1_EXTSEL_0 \
                                            | ADC_REG_TRIG_EXT_EDGE_DEFAULT)                      /*!< ADC group regular
                                           conversion trigger from external peripheral: LPTIM1 channel 1 event
                                           (capture compare: input capture or output capture).
                                           Trigger edge set to rising edge (default setting). */
#define LL_ADC_REG_TRIG_EXT_EXTI_LINE15    (ADC_CFGR1_EXTSEL_2 | ADC_CFGR1_EXTSEL_1 \
...
#define LL_ADC_SAMPLINGTIME_814CYCLES_5    (ADC_SMPR_SMP1_2 \
                                            | ADC_SMPR_SMP1_1 \
                                            | ADC_SMPR_SMP1_0) /*!< Sampling time 814.5 ADC clock cycles */
/**
  * @}
  */
...
#define LL_ADC_AWD_CH_VCORE_REG            ((LL_ADC_CHANNEL_VCORE      & ADC_CHANNEL_ID_MASK) \
                                            | ADC_CFGR1_AWD1EN | ADC_CFGR1_AWD1SGL) /*!< ADC analog watchdog monitoring
                                           of ADC internal channel connected to Vcore,
                                           converted by group regular only */
/**
  * @}
  */
...
/**
  ...
  *         @arg @ref LL_ADC_AWD_CH_VCORE_REG
  */
...
/**
  ...
  *         ADC must be disabled or enabled without conversion on going
  *         on group regular.
  * @rmtoll CFGR     WAIT           LL_ADC_SetLPModeAutoWait
  * @param  ADCx ADC instance
...
/**
  ...
  *         correspond to the current voltage level on the selected
  *         ADC channel.
  * @rmtoll CFGR     WAIT           LL_ADC_GetLPModeAutoWait
  * @param  ADCx ADC instance
  * @retval Returned value can be one of the following values:
  *         @arg @ref LL_ADC_LP_AUTOWAIT_DISABLE
  *         @arg @ref LL_ADC_LP_AUTOWAIT_ENABLE
...
/* Definitions for backward compatibility with legacy STM32 series */
#define LL_ADC_SetLowPowerMode LL_ADC_SetLPModeAutoWait /* Redefinition for legacy purpose */
#define LL_ADC_GetLowPowerMode LL_ADC_GetLPModeAutoWait /* Redefinition for legacy purpose */
...
{
  MODIFY_REG(ADCx->PWRR, ADC_PWRR_AUTOFF, LowPowerMode);
}
...
/**
  ...
  *           (with startup time between trigger and start of sampling).
  *           This feature can be combined with low power mode "auto wait".
  * @rmtoll PWRR     AUTOFF         LL_ADC_GetLPModeAutoPowerOff
  * @param  ADCx ADC instance
  * @retval Returned value can be one of the following values:
  *         @arg @ref LL_ADC_LP_AUTOPOWEROFF_DISABLE
  *         @arg @ref LL_ADC_LP_AUTOPOWEROFF_ENABLE
...
{
  return (uint32_t)(READ_BIT(ADCx->PWRR, ADC_PWRR_AUTOFF));
}
...
{
  MODIFY_REG(ADCx->PWRR, ADC_PWRR_DPD, LowPowerMode);
}
...
{
  return (uint32_t)(READ_BIT(ADCx->PWRR, ADC_PWRR_DPD));
}
                                                     ...
                                                     uint32_t AWDThresholdLowValue)
{
  ...
  MODIFY_REG(*preg,
             ...
             (AWDThresholdHighValue << ADC_AWD1TR_HT1_Pos) | AWDThresholdLowValue);
}
...
{
  SET_BIT(ADCx->IER, LL_ADC_FLAG_LDORDY);
}
...
/**
  ...
  * @rmtoll IER      LDORDYIE       LL_ADC_DisableIT_LDORDY
  * @param  ADCx ADC instance
  * @retval State of bit (1 or 0).
  */
__STATIC_INLINE void LL_ADC_DisableIT_LDORDY(ADC_TypeDef *ADCx)
{
  CLEAR_BIT(ADCx->IER, LL_ADC_FLAG_LDORDY);
}
...
/**
  * @brief  Get state of interruption ADC internal voltage regulator ready.
...
{
  return ((READ_BIT(ADCx->IER, LL_ADC_FLAG_LDORDY) == (LL_ADC_FLAG_LDORDY)) ? 1UL : 0UL);
}
